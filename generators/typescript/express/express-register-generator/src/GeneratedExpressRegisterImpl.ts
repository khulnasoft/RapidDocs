import { PackageId, convertHttpPathToExpressRoute, getTextOfTsNode } from "@rapiddocs-typescript/commons";
import { ExpressContext, GeneratedExpressRegister } from "@rapiddocs-typescript/contexts";
import { PackageResolver } from "@rapiddocs-typescript/resolvers";
import { partition } from "lodash-es";
import { ts } from "ts-morph";

import { IntermediateRepresentation, Name, Package } from "@rapiddocs-rapiddocs/ir-sdk/api";
...
export class GeneratedExpressRegisterImpl implements GeneratedExpressRegister {
    ...
    private getReferenceToServiceArgument(
        ...
        const referenceToPackage = this.packageResolver
            ...
            .rapiddocsFilepath.packagePath.reduce<ts.Expression>(
                (acc, part) =>
    ...
    private getServiceKey(packageId: PackageId) {
        ...
        return subpackage.rapiddocsFilepath.file != null ? subpackage.rapiddocsFilepath.file.camelCase.unsafeName : "_root";
    }
    ...
    private constructLiteralTypeNodeForServicesTree(
        ...
        const [leaves, otherChildren] = partition(
            root.subpackages,
            (subpackageId) => this.packageResolver.resolveSubpackage(subpackageId).rapiddocsFilepath.file != null
        );
    ...
    private getImportAliasForService(packageId: PackageId): string {
        ...
        return [
            ...package_.rapiddocsFilepath.packagePath.map((part) => part.camelCase.unsafeName),
            package_.rapiddocsFilepath.file != null
                ? `${package_.rapiddocsFilepath.file.pascalCase.unsafeName}Service`
                : "RootService"
        ].join("_");
    }
}
