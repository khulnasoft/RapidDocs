...
import { Audiences, RAPIDDOCS_PACKAGE_MARKER_FILENAME, generatorsYml } from "@khulnasoft/configuration";
import { noop, visitObject } from "@khulnasoft/core-utils";
import { isGeneric } from "@khulnasoft/rapiddocs-definition-schema";
import {
    ExampleType,
    HttpEndpoint,
...
import { constructHttpPath, injectAutogeneratedExamples } from "@khulnasoft/ir-utils";
...
import { RapiddocsFileContext, constructRapiddocsFileContext, constructRootApiFileContext } from "./RapiddocsFileContext";
import { IdGenerator } from "./IdGenerator";
import { PackageTreeGenerator } from "./PackageTreeGenerator";
import { generateRapiddocsConstants } from "./converters/constants";
...
export function generateIntermediateRepresentation({
    ...
    const intermediateRepresentation: Omit<IntermediateRepresentation, "sdkConfig" | "subpackages" | "rootPackage"> = {
        ...
        errors: {},
        services: {},
        constants: generateRapiddocsConstants(casingsGenerator),
    ...
    const visitDefinitionFile = (file: RapiddocsFileContext) => {
        packageTreeGenerator.addSubpackage(file.rapiddocsFilepath);

        visitObject(file.definitionFile, {
            imports: noop,
            docs: (docs) => {
                if (docs != null) {
                    packageTreeGenerator.addDocs(file.rapiddocsFilepath, docs);
                }
            },
            ...
            types: (types) => {
                ...
                for (const [typeName, typeDeclaration] of Object.entries(types)) {
                    ...
                    // rapiddocs definition users, but not actually meant to be in the
                    // generated SDKs
            ...
            webhooks: (webhooks) => {
                ...
                const webhookGroupId = IdGenerator.generateWebhookGroupId(file.rapiddocsFilepath);
                ...
                packageTreeGenerator.addWebhookGroup(webhookGroupId, file.rapiddocsFilepath);
            },
            channel: (channel) => {
                if (channel == null) {
                    return;
                }
                const websocketChannelId = IdGenerator.generateWebSocketChannelId(file.rapiddocsFilepath);
                ...
                if (intermediateRepresentation.websocketChannels != null) {
                    ...
                    packageTreeGenerator.addWebSocketChannel(websocketChannelId, file.rapiddocsFilepath);
                }
            }
        });
    };
    ...
    visitAllPackageMarkers(workspace, (relativeFilepath, packageMarker) => {
        ...
        if (typeof packageMarker.navigation === "string") {
            packageTreeGenerator.addPackageRedirection({
                from: convertToRapiddocsFilepath({ relativeFilepath, casingsGenerator }),
                to: convertToRapiddocsFilepath({
            ...
            if (relativeFilepath === RAPIDDOCS_PACKAGE_MARKER_FILENAME) {
    ...
    const sdkConfig: SdkConfig = {
        ...
        platformHeaders: {
            language: "X-Rapiddocs-Language",
            sdkName: "X-Rapiddocs-SDK-Name",
            sdkVersion: "X-Rapiddocs-SDK-Version",
            userAgent:
