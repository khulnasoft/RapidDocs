...
import { docsYml, getColorFromRawConfig, getColorType } from "@khulnasoft/configuration-loader";
...
export function validateTheme(
    ...
    if (accentPrimaryColor == null) {
        ...
        if (ratio < 3) {
            ...
            ruleViolations.push({
                severity: "warning",
                message: `The contrast ratio between the accent color and the background color for ${theme} mode is ${readableRatio}. Rapiddocs will adjust the color to meet the minimum contrast ratio of 4.5:1 for WCAG AA and 7:1 for WCAG AAA.`
            });
        } else if (ratio < 7) {
            ruleViolations.push({
                severity: "warning",
                message: `The contrast ratio between the accent color and the background color for ${theme} mode is ${readableRatio}. Rapiddocs will adjust the color to meet the minimum contrast ratio of 7:1 for WCAG AAA.`
            });
        }
    }
