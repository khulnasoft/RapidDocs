...
Add a `styles.css` file and include it in your `rapiddocs/` project:
...
<Note>
For customizing the background, logo, font, and layout of your Docs via Rapiddocs's built-in styling, 
check out the [Global Configuration](/learn/docs/getting-started/global-configuration). 
</Note>

## Custom JavaScript

Customize the behavior of your Docs site by injecting custom JavaScript globally. Add a `custom.js` file and include it in your `rapiddocs/` project:
...
You can use custom CSS and JS to replace Rapiddocs's default UI components with your own. The `header` and `footer` 
...
To implement your own components in Rapiddocs Docs, write JavaScript to render your 
custom components in the DOM. Build to CSS and JavaScript files that 
are stored in `rapiddocs/` and referenced in `docs.yml`:
...
See this [GitHub repo](https://github.com/khulnasoft/docs-custom-js-example) 
and its [generated docs page](https://custom-js-example.docs.buildwithrapiddocs.com/get-started/welcome) 
for an example of how to replace the Rapiddocs `header` and `footer` with custom React components. 
...
ReactDOM.render(
  ...
  document.getElementById('rapiddocs-header'),
)
```
...
ReactDOM.render(
  ...
  document.getElementById('rapiddocs-footer'),
)
```
...
- `yarn build` or `npm build` must generate files with deterministic names to be referenced in `docs.yml`. The above example uses a [`vite` config](https://github.com/khulnasoft/docs-custom-js-example/blob/main/custom-app/vite.config.ts) to accomplish this.
