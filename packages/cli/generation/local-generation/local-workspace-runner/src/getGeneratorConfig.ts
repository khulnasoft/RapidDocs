...
import {
    ...
    PypiOutput,
    RubyGemsOutput
} from "@rapiddocs-rapiddocs/fiddle-sdk/api";
import { RapiddocsGeneratorExec } from "@rapiddocs-rapiddocs/generator-exec-sdk";
import { EnvironmentVariable } from "@rapiddocs-rapiddocs/generator-exec-sdk/api";
...
): RapiddocsGeneratorExec.GithubPublishInfo | undefined {
    return githubPublishInfo != null
        ? FiddleGithubPublishInfo._visit<RapiddocsGeneratorExec.GithubPublishInfo | undefined>(githubPublishInfo, {
              npm: (value) =>
                  RapiddocsGeneratorExec.GithubPublishInfo.npm({
                      registryUrl: value.registryUrl,
                      packageName: value.packageName,
                      tokenEnvironmentVariable: EnvironmentVariable(value.token ?? "")
                  }),
              maven: (value) =>
                  RapiddocsGeneratorExec.GithubPublishInfo.maven({
                      registryUrl: value.registryUrl,
              ...
              pypi: (value) =>
                  RapiddocsGeneratorExec.GithubPublishInfo.pypi({
                      registryUrl: value.registryUrl,
                      packageName: value.packageName,
                      usernameEnvironmentVariable: EnvironmentVariable(value.credentials?.username ?? ""),
                      passwordEnvironmentVariable: EnvironmentVariable(value.credentials?.password ?? "")
              ...
              rubygems: (value) =>
                  RapiddocsGeneratorExec.GithubPublishInfo.rubygems({
                      registryUrl: value.registryUrl,
                      packageName: value.packageName,
                      apiKeyEnvironmentVariable: EnvironmentVariable(value.apiKey ?? "")
                  }),
              postman: (value) =>
                  RapiddocsGeneratorExec.GithubPublishInfo.postman({
                      apiKeyEnvironmentVariable: EnvironmentVariable(value.apiKey ?? ""),
                      workspaceIdEnvironmentVariable: EnvironmentVariable(value.workspaceId ?? "")
                  }),
              nuget: (value) =>
                  RapiddocsGeneratorExec.GithubPublishInfo.nuget({
                      registryUrl: value.registryUrl,
                      packageName: value.packageName,
                      apiKeyEnvironmentVariable: EnvironmentVariable(value.apiKey ?? "")
                  }),
...
export function getGeneratorConfig({
    ...
    const binds: string[] = [];
    const output = generatorInvocation.outputMode._visit<RapiddocsGeneratorExec.GeneratorOutputConfig>({
        publish: (value) => {
            return {
        ...
        downloadFiles: () => {
            return {
                mode: RapiddocsGeneratorExec.OutputMode.downloadFiles(),
                path: DOCKER_CODEGEN_OUTPUT_DIRECTORY,
                publishingMetadata: generatorInvocation.publishMetadata
            };
        },
        github: (value) => {
            const outputConfig: RapiddocsGeneratorExec.GeneratorOutputConfig = {
        ...
        _other: () => {
            throw new Error("Output type did not match any of the types supported by Rapiddocs");
        }
    });
    return {
        binds,
...
function newDummyPublishOutputConfig(
...
): RapiddocsGeneratorExec.GeneratorOutputConfig {
    ...
    return {
        mode: RapiddocsGeneratorExec.OutputMode.publish({
            registries: {
                maven: {
                    group: "",
