...
import { wrapWithHttps } from "@khulnasoft/docs-resolver";
import { DocsV1Read, DocsV2Read, RapiddocsNavigation } from "@khulnasoft/fdr-sdk";
import { AbsoluteFilePath, dirname, doesPathExist } from "@khulnasoft/fs-utils";
...
export async function runPreviewServer({
    ...
    if (bundlePath != null) {
        ...
        try {
            ...
            if (url == null) {
                throw new Error(
                    "Failed to connect to the docs preview server. Please contact support@buildwithrapiddocs.com"
                );
            }
            ...
            if (await doesPathExist(pathToBundle)) {
                ...
                context.logger.warn("Please reach out to support@buildwithrapiddocs.com.");
                return;
            }
        }
    }

    const absoluteFilePathToRapiddocs = dirname(initialProject.config._absolutePath);
    ...
    const watcher = new Watcher([absoluteFilePathToRapiddocs, ...additionalFilepaths], {
        recursive: true,
        ignoreInitial: true,
        debounce: 100,
    ...
    app.post("/v2/registry/docs/load-with-url", async (_, res) => {
        try {
            ...
            const response: DocsV2Read.LoadDocsForUrlResponse = {
                ...
                orgId: RapiddocsNavigation.OrgId(initialProject.config.organization)
            };
            res.send(response);
        } catch (error) {
