...
import { getRapiddocsDirectory } from "@khulnasoft/configuration-loader";
import {
    AbsoluteFilePath,
    Directory,
    File,
...
export const migration: Migration = {
    ...
    run: async ({ context }) => {
        ...
        const { files, directories } = await getFilesAndDirectories(absolutePathToRapiddocsDirectory);
        ...
        if (apisDirectory == null) {
            ...
            await addApiConfigurationToSingleWorkspace({
                ...
                absoluteFilepathToWorkspace: absolutePathToRapiddocsDirectory
            });
        } else {
            // Multiple workspaces
        ...
        if (openapiDirectory != null && openapiDirectory.contents[0] != null) {
            ...
            await writeFile(
                ...
                yaml.dump({
                    api: {
                        path: join(
                            await relativize(absolutePathToRapiddocsDirectory, absoluteFilepathToWorkspace),
