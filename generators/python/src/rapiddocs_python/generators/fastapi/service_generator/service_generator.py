...
class ServiceGenerator:
    _INIT_RAPIDDOCS_ROUTER_ARGUMENT = "router"
    ...
    def generate(
        ...
        self._add_init_rapiddocs_method(class_declaration=class_declaration)
        for endpoint_generator in self._endpoint_generators:
    ...
    def _generate_empty_class_declaration(
        ...
        return AST.ClassDeclaration(
            name=class_name,
            extends=[self._context.core_utilities.AbstractRapiddocsService()],
    ...
    def _add_init_rapiddocs_method(self, class_declaration: AST.ClassDeclaration) -> None:
        class_declaration.add_method(
            decorator=AST.ClassMethodDecorator.CLASS_METHOD,
            declaration=AST.FunctionDeclaration(
                name=self._context.core_utilities.INIT_RAPIDDOCS_METHOD_NAME,
                signature=AST.FunctionSignature(
                    parameters=[
                        AST.FunctionParameter(
                            name=ServiceGenerator._INIT_RAPIDDOCS_ROUTER_ARGUMENT,
                            type_hint=AST.TypeHint(type=FastAPI.APIRouter.REFERENCE),
                        )
                    ],
                    return_type=AST.TypeHint.none(),
                ),
                body=AST.CodeWriter(self._write_init_rapiddocs_body),
            ),
        )

    def _write_init_rapiddocs_body(self, writer: AST.NodeWriter) -> None:
        for endpoint_generator in self._endpoint_generators:
            writer.write_node(
                endpoint_generator.invoke_init_method(
                    reference_to_fastapi_router=AST.Expression(ServiceGenerator._INIT_RAPIDDOCS_ROUTER_ARGUMENT)
                )
            )
            writer.write_line()
