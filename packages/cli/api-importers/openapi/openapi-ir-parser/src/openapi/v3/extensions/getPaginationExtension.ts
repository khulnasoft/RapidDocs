...
import { Pagination } from "@khulnasoft/openapi-ir";
...
export function getRapiddocsPaginationExtension(
    document: OpenAPIV3.Document,
    operation: OpenAPIV3.OperationObject
): Pagination | undefined {
    const pagination = getExtension<Raw.PaginationExtensionSchema>(operation, RapiddocsOpenAPIExtension.PAGINATION);
    if (pagination == null) {
        return undefined;
    }
    if (typeof pagination === "boolean") {
        ...
        const topLevelPagination = getExtension<Raw.PaginationExtensionSchema>(
            document,
            RapiddocsOpenAPIExtension.PAGINATION
        );
