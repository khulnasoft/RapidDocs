...
import { createFdrGeneratorsSdkService } from "@khulnasoft/core";
...
function getMaybeBadVersionMessage(
    ...
    if (!isVersionAhead(cliVersion, minCliVersion)) {
        return [
            {
                severity: "fatal",
                message: `The generator ${generatorName} requires CLI version ${minCliVersion} or later (current version: ${cliVersion}). Please run \`rapiddocs upgrade\` to upgrade your CLI version and use this generator.`
            }
        ];
    }
    return;
