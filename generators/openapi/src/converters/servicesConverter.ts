    ...
    const tag =
        mode === "stoplight"
            ? (httpService.displayName ??
              httpService.name.rapiddocsFilepath.allParts.map((name) => name.originalName).join(" "))
            : httpService.name.rapiddocsFilepath.allParts.map((name) => name.pascalCase.unsafeName).join("");
    const operationObject: OpenAPIV3.OperationObject = {
        description: httpEndpoint.docs ?? undefined,
        operationId: [
            ...httpService.name.rapiddocsFilepath.allParts.map((name) => name.camelCase.unsafeName),
