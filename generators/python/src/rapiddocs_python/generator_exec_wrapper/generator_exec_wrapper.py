...
from rapiddocs.generator_exec.client import RapiddocsGeneratorExec
from rapiddocs.generator_exec.config import GeneratorConfig, RemoteGeneratorEnvironment
from rapiddocs.generator_exec.logging import GeneratorUpdate, TaskId


class GeneratorExecWrapper:
    def __init__(self, generator_config: GeneratorConfig):
        self.generator_exec_client: typing.Optional[RapiddocsGeneratorExec] = None
    ...
    def _init_remote(self, env: RemoteGeneratorEnvironment) -> None:
        self.generator_exec_client = RapiddocsGeneratorExec(base_url=env.coordinator_url_v_2)
        self.task_id = env.id
        if self._is_in_development(env):
            self.fdr_client = FdrClient(base_url="https://registry-dev2.buildwithrapiddocs.com")
        else:
    ...
    def _is_in_development(self, env: RemoteGeneratorEnvironment) -> bool:
        return env.coordinator_url_v_2.endswith("dev2.buildwithrapiddocs.com")
