...
import { RapiddocsOpenAPIExtension, OpenAPIExtension } from "@khulnasoft/openapi-ir-parser";
...
export async function loadOpenAPI({
    ...
    if (absolutePathToOpenAPIOverrides != null) {
        ...
        (parsed as any)[RapiddocsOpenAPIExtension.OPENAPI_OVERIDES_FILEPATH] != null
    ) {
        overridesFilepath = join(
            dirname(absolutePathToOpenAPI),
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            RelativeFilePath.of((parsed as any)[RapiddocsOpenAPIExtension.OPENAPI_OVERIDES_FILEPATH])
        );
    }
