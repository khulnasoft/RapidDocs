...
import { EnumSchema, SecurityScheme, Source } from "@khulnasoft/openapi-ir";
...
function convertSecuritySchemeHelper(
    ...
    if (securityScheme.type === "apiKey" && securityScheme.in === "header") {
        ...
        const headerNames = getExtension<HeaderSecuritySchemeNames>(
            securityScheme,
            RapiddocsOpenAPIExtension.RAPIDDOCS_HEADER_AUTH
        );
        return SecurityScheme.header({
            headerName: securityScheme.name,
            prefix: bearerFormat != null ? "Bearer" : headerNames?.prefix,
            headerVariableName:
                headerNames?.name ?? getExtension<string>(securityScheme, RapiddocsOpenAPIExtension.HEADER_VARIABLE_NAME),
            headerEnvVar: headerNames?.env
        });
    } else if (securityScheme.type === "http" && securityScheme.scheme === "bearer") {
        const bearerNames = getExtension<SecuritySchemeNames>(securityScheme, RapiddocsOpenAPIExtension.RAPIDDOCS_BEARER_TOKEN);
        return SecurityScheme.bearer({
            tokenVariableName:
                bearerNames?.name ??
                getExtension<string>(securityScheme, RapiddocsOpenAPIExtension.BEARER_TOKEN_VARIABLE_NAME),
...
function getScopes(oauthSecurityScheme: OpenAPIV3.OAuth2SecurityScheme, source: Source): EnumSchema | undefined {
    ...
    if (scopes != null) {
        const schemaWithExample = convertEnum({
            ...
            rapiddocsEnum: Object.fromEntries(
