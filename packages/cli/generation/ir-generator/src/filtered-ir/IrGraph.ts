import { Audiences as ConfigAudiences } from "@khulnasoft/configuration";
import { assertNever, noop } from "@khulnasoft/core-utils";
...
export class IrGraph {
    ...
    public addType({
        ...
        descendantFilepaths: Set<RapiddocsFilepath>;
    }): void {
    ...
    public markTypeForAudiences(declaredTypeName: DeclaredTypeName, audiences: string[]): void {
        ...
        if (this.hasAudience(audiences)) {
            ...
            this.addSubpackages(declaredTypeName.rapiddocsFilepath);
            this.types[typeId]?.referencedSubpackages.forEach((rapiddocsFilePath) => {
                this.addSubpackages(rapiddocsFilePath);
            });
        }
    }
    ...
    public addError(errorDeclaration: ErrorDeclaration): void {
        ...
        const referencedSubpackages = new Set<RapiddocsFilepath>();
    ...
    public addEndpoint(
        ...
        httpEndpoint.errors.forEach((responseError) => {
            ...
            referencedSubpackages.add(responseError.error.rapiddocsFilepath);
        });
    ...
    public markEndpointForAudience(
        ...
        if (this.hasAudience(audiences)) {
            ...
            httpEndpoints.forEach((httpEndpoint) => {
                ...
                this.endpoints[endpointId]?.referencedSubpackages.forEach((rapiddocsFilePath) => {
                    this.addSubpackages(rapiddocsFilePath);
                });
            });
            this.servicesNeededForAudience.add(serviceId);
            this.addSubpackages(declaredServiceName.rapiddocsFilepath);
        }
    }

    public addWebhook(file: RapiddocsFileContext, webhook: Webhook, rawWebhook?: RawSchemas.WebhookSchema): void {
        ...
        referencedSubpackages.add(file.rapiddocsFilepath);
    ...
    public markWebhookForAudiences(file: RapiddocsFileContext, webhook: Webhook, audiences: AudienceId[]): void {
        const webhookId = webhook.id;
        if (webhookId == null) {
            return;
        }
        ...
        if (this.hasAudience(audiences)) {
            ...
            this.addSubpackages(file.rapiddocsFilepath);
        }
    }
    ...
    private addSubpackages(rapiddocsFilePath: RapiddocsFilepath): void {
        for (let i = 1; i <= rapiddocsFilePath.allParts.length; ++i) {
            const packageFilePath = {
                ...rapiddocsFilePath,
                allParts: rapiddocsFilePath.allParts.slice(0, i)
            };
...
function populateReferencesFromTypeReference(
    ...
    referencedSubpackages: Set<RapiddocsFilepath>
) {
...
function populateReferencesFromTypeName(
    ...
    referencedSubpackages.add(typeName.rapiddocsFilepath);
}
