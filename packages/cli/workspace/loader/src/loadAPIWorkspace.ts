import { Spec } from "@khulnasoft/api-workspace-commons";
import {
...
import {
    ConjureWorkspace,
    LazyRapiddocsWorkspace,
    OSSWorkspace,
    WorkspaceLoader,
    WorkspaceLoaderFailureType
} from "@khulnasoft/lazy-rapiddocs-workspace";
...
export async function loadAPIWorkspace({
    ...
    if (await doesPathExist(join(absolutePathToWorkspace, RelativeFilePath.of(DEFINITION_DIRECTORY)))) {
        const rapiddocsWorkspace = new LazyRapiddocsWorkspace({
            absoluteFilePath: absolutePathToWorkspace,
            generatorsConfiguration,
            workspaceName,
            changelog,
