import { RapiddocsContainerRegex } from "./visitRawTypeReference";
...
function isFileArray(typeReference: string): boolean {
    const listMatch = typeReference.match(RapiddocsContainerRegex.LIST);
...
export function parseRawFileType(typeReference: string): RawFileType | undefined {
    ...
    const optionalMatch = typeReference.match(RapiddocsContainerRegex.OPTIONAL);
