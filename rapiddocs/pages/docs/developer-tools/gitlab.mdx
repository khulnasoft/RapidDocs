...
To host your Rapiddocs docs using GitLab, you will need to [add a Rapiddocs token to your repository variables](/learn/docs/developer-tools/gitlab#add-a-token-to-gitlab). 
...
To add this to your GitLab Rapiddocs project, create a `.gitlab-ci.yml` file in the root of your repository.
...
preview_docs:
  ...
  rules:
    - if: '$CI_PIPELINE_SOURCE == "merge_request_event"'
  script:
    - echo "Running rapiddocs generate --docs --preview..."
    - |
      OUTPUT=$(rapiddocs generate --docs --preview) || true
      echo "$OUTPUT"
...
publish_sdks:
  ...
  script: 
      ...
      - rapiddocs generate --group ts-sdk --version $VERSION --log-level debug
```
</Accordion>
...
<Steps>
### Contact us
To get set up with a GitLab pipeline to preview your docs automatically, reach out via your dedicated Slack channel or [email](mailto:support@buildwithrapiddocs.com). 
### Log in
Log into [GitLab](https://gitlab.com/users/sign_in).
