...
export class DynamicTypeMapper {
    ...
    private convertLiteral({ literal }: { literal: RapiddocsIr.dynamic.LiteralType }): go.Type {
        switch (literal.type) {
            case "boolean":
                return go.Type.bool();
            case "string":
    ...
    private convertNamed({ named }: { named: RapiddocsIr.dynamic.NamedType }): go.Type {
        const goTypeReference = go.Type.reference(
            go.typeReference({
                name: this.context.getTypeName(named.declaration.name),
                importPath: this.context.getImportPath(named.declaration.rapiddocsFilepath)
            })
        );
        switch (named.type) {
            case "alias":
    ...
    private convertPrimitive({ primitive }: { primitive: RapiddocsIr.PrimitiveTypeV1 }): go.Type {
        switch (primitive) {
            case "INTEGER":
                return go.Type.int();
            case "UINT":
