...
import { RapiddocsDefinition, RapiddocsWorkspace } from "@khulnasoft/api-workspace-commons";
import { dependenciesYml } from "@khulnasoft/configuration-loader";
import { createFiddleService } from "@khulnasoft/core";
import { assertNever, noop, visitObject } from "@khulnasoft/core-utils";
import { RootApiFileSchema, YAML_SCHEMA_VERSION } from "@khulnasoft/rapiddocs-definition-schema";
import { AbsoluteFilePath, RelativeFilePath, doesPathExist, join } from "@khulnasoft/fs-utils";
import { parseVersion } from "@khulnasoft/semver-utils";
...
export async function loadDependency({
    ...
    let definition: RapiddocsDefinition | undefined;
...
async function validateLocalDependencyAndGetDefinition({
    ...
    cliVersion: string;
    settings?: OSSWorkspace.Settings;
}): Promise<RapiddocsDefinition | undefined> {
...
async function validateVersionedDependencyAndGetDefinition({
    ...
    let metadata: RapiddocsFiddle.Api;
    if (!(await doesPathExist(pathToDefinition)) || !(await doesPathExist(pathToMetadata))) {
        ...
        const response = await FIDDLE.definitionRegistry.get(
            RapiddocsFiddle.OrganizationId(dependency.organization),
            RapiddocsFiddle.ApiId(dependency.apiName),
            dependency.version
        );
        if (!response.ok) {
        ...
        if (parsedYamlVersionOfDependency != null) {
            if (parsedYamlVersionOfDependency > YAML_SCHEMA_VERSION) {
                context.failWithoutThrowing(
                    `${dependency.organization}/${dependency.apiName}@${dependency.version} on a higher version of rapiddocs. Upgrade this workspace to ${response.body.cliVersion}`
                );
                return undefined;
            } else if (parsedYamlVersionOfDependency < YAML_SCHEMA_VERSION) {
                context.failWithoutThrowing(
                    `${dependency.organization}/${dependency.apiName}@${dependency.version} on a lower version of rapiddocs. Upgrade it to ${cliVersion}`
                );
                return undefined;
            }
        }
    ...
    if (workspaceOfDependency.type === "oss") {
        context.failWithoutThrowing("Dependency must be a rapiddocs workspace.");
        return undefined;
    }
...
async function getAreRootApiFilesEquivalent(
    rootApiFile: RootApiFileSchema,
    workspaceOfDependency: RapiddocsWorkspace
