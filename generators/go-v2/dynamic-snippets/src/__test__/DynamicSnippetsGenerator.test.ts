import { DynamicSnippetsTestRunner } from "@khulnasoft/browser-compatible-base-generator";
    ...
    runner.runTests({
        buildGenerator: ({ irFilepath }) =>
            buildDynamicSnippetsGenerator({
                irFilepath,
                config: buildGeneratorConfig({ customConfig: { exportedClientName: "RapiddocsClient" } })
            })
    });
});
