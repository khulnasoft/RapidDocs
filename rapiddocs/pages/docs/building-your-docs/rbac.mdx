...
Rapiddocs allows you to restrict parts of your navigation to individuals with specific roles. Below, we walk through each of the steps
...
In this step, we will configure authentication so that Rapiddocs can understand what roles a particular user has. Rapiddocs expects the user's
...
Upon login, you must set a JWT for the user using a secret key that we will provide you with. The JWT must have a `rapiddocs` claim
...
<Note>Please reach out to support@buildwithrapiddocs.com when you are on this step so we can provide you with a secret key.</Note>
