...
Instead of forcing SDK users to learn the intricacies of your pagination system, Rapiddocs SDKs will return an iterator so that users can simply loop through all the results.
...
Rapiddocs supports the following pagination schemes:
...
Annotate the desired paginated endpoint with the `x-rapiddocs-pagination` extension.
