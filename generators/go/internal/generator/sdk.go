...
import (
	...
	"github.com/khulnasoft/rapiddocs-go/internal/gospec"
	generatorexec "github.com/khulnasoft/generator-exec-go"
)

// goLanguageHeader is the identifier used for the X-Rapiddocs-Language platform header.
...
func (f *fileWriter) WriteClient(
	...
	rapiddocsFilepath *ir.RapiddocsFilepath,
	...
	for _, irEndpoint := range irEndpoints {
		endpoint, err := f.endpointFromIR(rapiddocsFilepath, irEndpoint, environmentsConfig, errorDiscriminationStrategy, serviceHeaders, idempotencyHeaders, inlinePathParameters, inlineFileProperties, packageLayout)
		if err != nil {
			return nil, err
		}
	...
	if packageLayout == PackageLayoutFlat && rapiddocsFilepath.File != nil {
		clientName = rapiddocsFilepath.File.PascalCase.UnsafeName + "Client"
	...
	for _, subpackage := range subpackages {
		if packageLayout == PackageLayoutFlat {
			...
			var (
				importPath     = packagePathToImportPath(f.baseImportPath, packagePathForClient(subpackage.RapiddocsFilepath))
			...
			if errorDiscriminationByPropertyStrategy == nil {
				...
				for _, responseError := range endpoint.Errors {
					...
					if packageLayout == PackageLayoutFlat {
						...
						errorImportPath := rapiddocsFilepathToImportPath(f.baseImportPath, errorDeclaration.Name.RapiddocsFilepath)
...
func newEndpointSnippet(
	...
	methodName := getEndpointMethodName(rapiddocsFilepath, endpoint)
...
func getEndpointMethodName(
	...
	for _, packageElem := range rapiddocsFilepath.PackagePath {
		packageElements = append(packageElements, packageElem.PascalCase.UnsafeName)
	}
	if rapiddocsFilepath.File != nil {
		packageElements = append(packageElements, rapiddocsFilepath.File.PascalCase.UnsafeName)
	}
		...
		parameters = append(
			...
			&ast.StructType{
				Name: &ast.ImportedReference{
					...
					ImportPath: rapiddocsFilepathToImportPath(f.baseImportPath, rapiddocsFilepath),
				},
				Fields: fields,
			},
		)
...
func generatedClientInstantiation(
	...
	if packageLayout != PackageLayoutFlat {
		rootImportPath = packagePathToImportPath(baseImportPath, packagePathForClient(new(ir.RapiddocsFilepath)))
	}
	...
	if irEndpoint.SdkRequest != nil {
		if needsRequestParameter(irEndpoint, inlinePathParameters, inlineFileProperties) {
			...
			if irEndpoint.SdkRequest.Shape.Wrapper != nil {
				requestImportPath := rapiddocsFilepathToImportPath(f.baseImportPath, rapiddocsFilepath)
				if packageLayout == PackageLayoutFlat {
...
func (f *fileWriter) WriteError(errorDeclaration *ir.ErrorDeclaration) error {
	...
	var (
		importPath = rapiddocsFilepathToImportPath(f.baseImportPath, errorDeclaration.Name.RapiddocsFilepath)
