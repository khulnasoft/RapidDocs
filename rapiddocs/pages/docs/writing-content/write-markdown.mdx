...
description: Use Markdown and MDX to add content to your Rapiddocs documentation site, including Rapiddocs's built-in component library.
---
...
Place your pages inside your `rapiddocs/` folder and link to them from your [navigation settings](/learn/docs/building-your-docs/navigation) in the `docs.yml` file.
...
Rapiddocs automatically generates the `<h1>` page header for each page from `docs.yml`. For example, here's the `docs.yml` entry that maps the page you are reading now:
```yml
          - page: Write Markdown content
            path: ./docs/pages/rapiddocs-docs/content/write-markdown.mdx
```
...
## Rapiddocs components

Rapiddocs has a built-in component library you can use in Markdown. [Explore the components.](/learn/docs/content/components/overview)
...
### Link format
Use a `/` character to begin a relative URL to another page on your docs site. This routes to the `url` defined in your `docs.yml` file, such as `example-docs.buildwithrapiddocs.com`. For example, if you want to link to `https://example-docs.buildwithrapiddocs.com/overview/introduction`, you can write the link in Markdown as follows:
...
<embed src="../../../../rapiddocs/assets/changelogs/docs/embed-rapiddocs-waving.mp4" type="video/mp4" width="640px" height="360px" />
...
Rapiddocs supports [LaTeX](https://www.latex-project.org/) math equations. To use LaTeX, wrap your inline math equations in `$`. For example, `$(x^2 + y^2 = z^2)$` will render $x^2 + y^2 = z^2$.
...
## Diagrams
Rapiddocs supports creating diagrams within your Markdown using [Mermaid](https://mermaid.js.org/). Mermaid offers a variety of diagrams, including flowcharts, entity-relationship models, and Gantt charts. To include a Mermaid diagram in your Markdown file, create a codeblock marked with `mermaid`. 
