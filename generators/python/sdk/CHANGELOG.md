...
- Feat: The generator now respects returning nested properties, these can be specified via:
  ...
  my/endpoint:
    get:
      x-rapiddocs-sdk-return-value: jobId
      response: Job
  ```

  For a similar situation using the Rapiddocs definition:
...
- Internal: The SDK generator has now been upgraded to use Pydantic V2 internally. Note that
  ...
  an improvement in `rapiddocs generate` times.
...
- Improvement: The generated SDK now allows for specifying whether or not to generate `streaming` functions as overloaded functions or separate functions.
  ...
  The latter is configurable by specifying `stream-condition` and `response-stream` on your request within your Rapiddocs definition:
...
- Improvement: The generator now outputs an `exampleId` alongside each generated snippet so that
  ...
  Rapiddocs's API and making sure that you can show multiple snippets in the generated docs.
...
generators:
  - name: rapiddocsapi/rapiddocs-python-sdk
    metadata:
      ...
      authors:
        - email: support@buildwithrapiddocs.com
          name: Armando
```
...
- Improvement: Union types leverage the rapiddocs aware base model to include JSON and Dict function overrides.
...
- Fix: The generator no longer attempts to create a version file if Rapiddocs does not own generating the full package (e.g. in local generation). It's too confusing for to make the relevant changes to the package set up, and is also arguably not even needed in local generation.
...
- Beta: Introduce a `client` custom config that allows you to specify class_name and
  ...
  2. Add custom functions to your generated SDK:
     ...
     generated client. The easiest way to do this is to configure Rapiddocs to write
     the autogenerated client in another file and extend it on your own.

     With the configuration above, Rapiddocs's Python SDK generator will create a
...
- Fix: Python now supports API specifications that leverage lists for file upload. Previously, Rapiddocs incorrectly made all `list<file>` type requests simply `file`.
...
- Fix: Acknowledge the optionality of a `File` property, previously we were requiring all `File` type inputs, even if they were specified as optional within the OpenAPI or Rapiddocs definition. Now, we check if the parameter is required and make the parameter optional if it is not.
...
- Feature: If the auth scheme has environment variables specified,
  ...
  For example, for the following Rapiddocs Definition

  ```
  auth: APIKey
  auth-schemes:
    APIKey:
      header: X-RAPIDDOCS-API-KEY
      type: string
      env: RAPIDDOCS_API_KEY
  ```
  ...
  class Client:
    ...
    def __init__(self, *, apiKey: str = os.getenv("RAPIDDOCS_API_KEY"))
  ```
