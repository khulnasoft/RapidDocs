...
import { RapiddocsSeedConfig } from "./config";

export interface GeneratorWorkspace {
    workspaceName: string;
    absolutePathToWorkspace: AbsoluteFilePath;
    workspaceConfig: RapiddocsSeedConfig.SeedWorkspaceConfiguration;
}

export interface CliWorkspace {
    workspaceName: string;
    absolutePathToWorkspace: AbsoluteFilePath;
    workspaceConfig: RapiddocsSeedConfig.CliSeedWorkspaceConfiguration;
}

export const SEED_DIRECTORY = "seed";
export const SEED_CONFIG_FILENAME = "seed.yml";
export const CLI_SEED_DIRECTORY = "rapiddocs-cli";
...
export async function loadGeneratorWorkspaces(): Promise<GeneratorWorkspace[]> {
    ...
    for (const workspace of workspaceDirectoryNames) {
        ...
        workspaces.push({
            ...
            workspaceConfig: yaml.load(seedConfig.toString()) as any as RapiddocsSeedConfig.SeedWorkspaceConfiguration,
...
export async function getRapiddocsCliSeedDirectory(): Promise<AbsoluteFilePath | undefined> {
    const seedDirectoryStr = await findUp(SEED_DIRECTORY, { type: "directory" });
    if (seedDirectoryStr == null) {
        return undefined;
    }
...
export async function loadCliWorkspace(): Promise<CliWorkspace> {
    ...
    const seedConfig = await readFile(join(absolutePathToWorkspace, RelativeFilePath.of(SEED_CONFIG_FILENAME)));
    const workspaceConfig = yaml.load(seedConfig.toString()) as any as RapiddocsSeedConfig.CliSeedWorkspaceConfiguration;
