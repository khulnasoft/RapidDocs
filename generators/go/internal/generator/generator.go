...
import (
	...
	"github.com/khulnasoft/rapiddocs-go/internal/ast"
	"github.com/khulnasoft/rapiddocs-go/internal/coordinator"
	"github.com/khulnasoft/rapiddocs-go/internal/rapiddocs/ir"
	rapiddocsir "github.com/khulnasoft/rapiddocs-go/internal/rapiddocs/ir"
	generatorexec "github.com/khulnasoft/generator-exec-go"
)

const (
	// embeddedCoreImportPath is the import path for the core package
	// used in the embedded files.
	embeddedCoreImportPath = "github.com/khulnasoft/rapiddocs-go/internal/generator/sdk/core"
...
// a reference to a (potentially) modified HttpService, and its original RapiddocsFilepath.
//
// A service's RapiddocsFilepath is modified whenever it it represents an implicit root package
...
type SubpackageToGenerate struct {
	Subpackage           *rapiddocsir.Subpackage
	OriginalRapiddocsFilepath *rapiddocsir.RapiddocsFilepath
}

// NewSubpackagesToGenerate returns a slice of subpackages to generate from the given IR.
func NewSubpackagesToGenerate(ir *rapiddocsir.IntermediateRepresentation) []*SubpackageToGenerate {
	var subpackagesToGenerate []*SubpackageToGenerate
	for _, irSubpackage := range ir.Subpackages {
		originalRapiddocsFilepath := irSubpackage.RapiddocsFilepath
		if len(irSubpackage.Subpackages) > 0 && irSubpackage.RapiddocsFilepath.File != nil {
			// This represents a nested root package, so we need to deposit
			// the client in client subpackage (e.g. user/client).
			//
			// We do this by updating the RapiddocsFilepath for the subpackage
			// and service (if any).
			irSubpackage.RapiddocsFilepath = &rapiddocsir.RapiddocsFilepath{
				AllParts:    irSubpackage.RapiddocsFilepath.AllParts,
				PackagePath: append(irSubpackage.RapiddocsFilepath.PackagePath, irSubpackage.RapiddocsFilepath.File),
				File:        nil,
			}
			if irSubpackage.Service != nil {
				irService := ir.Services[*irSubpackage.Service]
				irService.Name.RapiddocsFilepath = &rapiddocsir.RapiddocsFilepath{
					AllParts:    irService.Name.RapiddocsFilepath.AllParts,
					PackagePath: append(irService.Name.RapiddocsFilepath.PackagePath, irService.Name.RapiddocsFilepath.File),
					File:        nil,
				}
			}
		}
		subpackagesToGenerate = append(
			...
			&SubpackageToGenerate{
				...
				OriginalRapiddocsFilepath: originalRapiddocsFilepath,
			},
		)
	}
...
func (g *Generator) generateModelTypes(ir *rapiddocsir.IntermediateRepresentation, mode Mode, rootClientInstantiation *ast.AssignStmt, rootPackageName string) ([]*File, []*GeneratedClient, error) {
	...
	for fileInfo, typesToGenerate := range fileInfoToTypes {
		...
		for _, typeToGenerate := range typesToGenerate {
			...
			case typeToGenerate.Endpoint != nil:
				if mode == ModeFiber {
					if err := writer.WriteFiberRequestType(typeToGenerate.RapiddocsFilepath, typeToGenerate.Endpoint, g.config.EnableExplicitNull); err != nil {
						return nil, nil, err
					}
				} else if mode == ModeClient {
					if err := writer.WriteRequestType(
						typeToGenerate.RapiddocsFilepath,
...
func (g *Generator) generate(ir *rapiddocsir.IntermediateRepresentation, mode Mode) ([]*File, error) {
	if g.config.ImportPath == "" {
		...
		for _, typeDeclaration := range ir.Types {
			typeImportPath := rapiddocsFilepathToImportPath(g.config.ImportPath, typeDeclaration.Name.RapiddocsFilepath)
			for _, referencedType := range declaredTypeNamesForTypeIDs(ir, typeDeclaration.ReferencedTypes) {
				referencedImportPath := rapiddocsFilepathToImportPath(g.config.ImportPath, referencedType.RapiddocsFilepath)
	...
	if g.config.PackageLayout == PackageLayoutFlat {
		for _, subpackage := range ir.Subpackages {
			if subpackage.RapiddocsFilepath.PackagePath != nil && len(subpackage.RapiddocsFilepath.PackagePath) > 0 {
				var packagePathElements []string
				for _, packagePathElement := range subpackage.RapiddocsFilepath.AllParts {
	...
	if cycleInfo != nil {
		for _, leafType := range cycleInfo.LeafTypes {
			// Update every leaf type's RapiddocsFilepath so that the rest of
			// the types reference it from the appropriate location.
			typeDecl := ir.Types[leafType]

			var packagePathElement *rapiddocsir.Name
			if packagePath := typeDecl.Name.RapiddocsFilepath.PackagePath; len(packagePath) > 0 {
				packagePathElement = packagePath[len(packagePath)-1]
			}

			newRapiddocsFilepath := &rapiddocsir.RapiddocsFilepath{
			...
			if packagePathElement != nil {
				newRapiddocsFilepath.AllParts = append(newRapiddocsFilepath.AllParts, packagePathElement)
				newRapiddocsFilepath.PackagePath = append(newRapiddocsFilepath.PackagePath, packagePathElement)
			}
			newRapiddocsFilepath.AllParts = append(newRapiddocsFilepath.AllParts, commonPackageElement)

			replaceFilepathForTypeInIR(ir, typeDecl.Name.TypeId, newRapiddocsFilepath)
		}
	}
	...
	if ir.RootPackage != nil && ir.RootPackage.Docs != nil && len(*ir.RootPackage.Docs) > 0 {
		fileInfo := fileInfoForPackage(rootPackageName, ir.RootPackage.RapiddocsFilepath)
		writer := newFileWriter(
			fileInfo.filename,
			fileInfo.packageName,
			"",
	...
	for _, subpackage := range ir.Subpackages {
		...
		fileInfo := fileInfoForPackageDocs(subpackage.RapiddocsFilepath)
		writer := newFileWriter(
			fileInfo.filename,
			fileInfo.packageName,
			"",
	...
	// Then split up all the types based on the Rapiddocs directory they belong to (i.e. the root package,
	...
	case ModeClient:
		...
		if ir.RootPackage != nil {
			var rootSubpackages []*rapiddocsir.Subpackage
			...
			if ir.RootPackage.Service != nil {
				file, generatedClient, err := g.generateRootService(
					...
					rootSubpackages,
					rootClientInstantiation,
					ir.RootPackage.RapiddocsFilepath,
					rootPackageName,
				)
				if err != nil {
					return nil, err
		...
		if g.config.PackageLayout != PackageLayoutFlat {
			for _, subpackageToGenerate := range subpackagesToGenerate {
				...
				var subpackages []*rapiddocsir.Subpackage
				...
				if irSubpackage.Service == nil {
					...
					file, err := g.generateServiceWithoutEndpoints(
						...
						subpackages,
						rootClientInstantiation,
						subpackageToGenerate.OriginalRapiddocsFilepath,
					)
					if err != nil {
						return nil, err
					}
...
func (g *Generator) generateRootService(
	...
	irService *rapiddocsir.HttpService,
	irSubpackages []*rapiddocsir.Subpackage,
	rootClientInstantiation *ast.AssignStmt,
	originalRapiddocsFilepath *rapiddocsir.RapiddocsFilepath,
	rootPackageName string,
) (*File, *GeneratedClient, error) {
	fileInfo := fileInfoForRootService(irService.Name.RapiddocsFilepath, rootPackageName, g.config.PackageLayout)
	writer := newFileWriter(
		fileInfo.filename,
		fileInfo.packageName,
		g.config.ImportPath,
...
func (g *Generator) generateService(
	...
	fileInfo := fileInfoForService(irService.Name.RapiddocsFilepath)
	writer := newFileWriter(
		fileInfo.filename,
		fileInfo.packageName,
		g.config.ImportPath,
...
func (g *Generator) generateServiceWithoutEndpoints(
	ir *rapiddocsir.IntermediateRepresentation,
	irSubpackage *rapiddocsir.Subpackage,
	irSubpackages []*rapiddocsir.Subpackage,
	rootClientInstantiation *ast.AssignStmt,
	originalRapiddocsFilepath *rapiddocsir.RapiddocsFilepath,
) (*File, error) {
	fileInfo := fileInfoForService(irSubpackage.RapiddocsFilepath)
	writer := newFileWriter(
		fileInfo.filename,
		fileInfo.packageName,
		g.config.ImportPath,
...
func (g *Generator) generateRootServiceWithoutEndpoints(
	...
	fileInfo := fileInfoForRootService(rapiddocsFilepath, rootPackageName, g.config.PackageLayout)
	writer := newFileWriter(
		fileInfo.filename,
		fileInfo.packageName,
		g.config.ImportPath,
...
func readIR(irFilename string) (*rapiddocsir.IntermediateRepresentation, error) {
	bytes, err := os.ReadFile(irFilename)
	if err != nil {
		return nil, fmt.Errorf("failed to read intermediate representation: %v", err)
	}
	ir := new(rapiddocsir.IntermediateRepresentation)
...
// declared type names.
func declaredTypeNamesForTypeIDs(ir *rapiddocsir.IntermediateRepresentation, typeIDs []rapiddocsir.TypeId) []*rapiddocsir.DeclaredTypeName {
	result := make([]*rapiddocsir.DeclaredTypeName, 0, len(typeIDs))
...
func fileInfoForType(rootPackageName string, rapiddocsFilepath *rapiddocsir.RapiddocsFilepath) fileInfo {
	var packages []string
	for _, packageName := range rapiddocsFilepath.PackagePath {
		packages = append(packages, strings.ToLower(packageName.CamelCase.SafeName))
	}
	basename := "types"
	if rapiddocsFilepath.File != nil {
		basename = rapiddocsFilepath.File.SnakeCase.UnsafeName
	}
	if len(packages) == 0 {
		return fileInfo{
...
func fileInfoForPackageDocs(rapiddocsFilepath *rapiddocsir.RapiddocsFilepath) *fileInfo {
	packagePath := packagePathForDocs(rapiddocsFilepath)
	return &fileInfo{
...
func fileInfoForRootService(rapiddocsFilepath *rapiddocsir.RapiddocsFilepath, rootPackageName string, packageLayout PackageLayout) *fileInfo {
	if packageLayout != PackageLayoutFlat {
		return fileInfoForService(rapiddocsFilepath)
	}
	return &fileInfo{
		filename:    "client.go",
		packageName: rootPackageName,
...
func fileInfoForService(rapiddocsFilepath *rapiddocsir.RapiddocsFilepath) *fileInfo {
	packagePath := packagePathForClient(rapiddocsFilepath)
	return &fileInfo{
...
func fileInfoForPackage(rootPackageName string, rapiddocsFilepath *rapiddocsir.RapiddocsFilepath) *fileInfo {
	var packages []string
	for _, packageName := range rapiddocsFilepath.PackagePath {
		packages = append(packages, strings.ToLower(packageName.CamelCase.SafeName))
	}
...
func generatedNamesFromIR(ir *rapiddocsir.IntermediateRepresentation) map[string]struct{} {
...
func generatedPackagesFromIR(ir *rapiddocsir.IntermediateRepresentation) map[string]struct{} {
	generatedPackages := make(map[string]struct{})
	for _, irService := range ir.Services {
		rapiddocsFilepath := irService.Name.RapiddocsFilepath
		if rapiddocsFilepath.File != nil {
			generatedPackages[strings.ToLower(rapiddocsFilepath.File.CamelCase.SafeName)] = struct{}{}
		}
	}
...
func shouldSkipRequestType(irEndpoint *rapiddocsir.HttpEndpoint, inlinePathParameters bool, inlineFileProperties bool) bool {
...
// one body property.
func fileUploadHasBodyProperties(fileUpload *rapiddocsir.FileUploadRequest) bool {
	if fileUpload == nil {
		return false
	}
...
// one file property.
func fileUploadHasFileProperties(fileUpload *rapiddocsir.FileUploadRequest) bool {
	if fileUpload == nil {
		return false
	}
	// If this request is a file upload, there must be at least one body property
...
func packagePathForDocs(rapiddocsFilepath *rapiddocsir.RapiddocsFilepath) []string {
	var packages []string
	for _, packageName := range rapiddocsFilepath.PackagePath {
		packages = append(packages, strings.ToLower(packageName.CamelCase.SafeName))
	}
	...
	directory := strings.ToLower(rapiddocsFilepath.File.CamelCase.SafeName)
	return append(packages, directory)
}

func packagePathForClient(rapiddocsFilepath *rapiddocsir.RapiddocsFilepath) []string {
	var packages []string
	for _, packageName := range rapiddocsFilepath.PackagePath {
		packages = append(packages, strings.ToLower(packageName.CamelCase.SafeName))
	}
...
type typeToGenerate struct {
	...
	TypeDeclaration *rapiddocsir.TypeDeclaration
	Endpoint        *rapiddocsir.HttpEndpoint
	Service         *rapiddocsir.HttpService
}

// fileInfoToTypes consolidates all of the given types based on the file they will be generated into.
func fileInfoToTypes(
	rootPackageName string,
	irTypes map[rapiddocsir.TypeId]*rapiddocsir.TypeDeclaration,
	irServices map[rapiddocsir.ServiceId]*rapiddocsir.HttpService,
	irServiceTypeReferenceInfo *rapiddocsir.ServiceTypeReferenceInfo,
	...
	for _, irService := range irServices {
		fileInfo := fileInfoForType(rootPackageName, irService.Name.RapiddocsFilepath)
		if packageLayout == PackageLayoutFlat && irService.Name.RapiddocsFilepath.File != nil {
	...
	if irServiceTypeReferenceInfo == nil {
		...
		for _, irType := range irTypes {
			fileInfo := fileInfoForType(rootPackageName, irType.Name.RapiddocsFilepath)
			result[fileInfo] = append(
				result[fileInfo],
				&typeToGenerate{
					ID:              irType.Name.TypeId,
					RapiddocsFilepath:    irType.Name.RapiddocsFilepath,
		...
		for _, typeId := range sharedTypes {
			...
			fileInfo := fileInfoForType(rootPackageName, typeDeclaration.Name.RapiddocsFilepath)
		...
		for serviceId, typeIds := range irServiceTypeReferenceInfo.TypesReferencedOnlyByService {
			...
			for _, typeId := range typeIds {
				...
				fileInfo := fileInfoForType(rootPackageName, typeDeclaration.Name.RapiddocsFilepath)
				if shouldSetFileInfoToMatchService(typeDeclaration.Name.RapiddocsFilepath, service.Name.RapiddocsFilepath) {
					fileInfo.filename = filepath.Join(filepath.Dir(fileInfo.filename), service.Name.RapiddocsFilepath.File.SnakeCase.UnsafeName+".go")
				}
				result[fileInfo] = append(
					result[fileInfo],
					&typeToGenerate{
...
func shouldSetFileInfoToMatchService(
	typeRapiddocsFilepath *rapiddocsir.RapiddocsFilepath,
	serviceRapiddocsFilepath *rapiddocsir.RapiddocsFilepath,
) bool {
	if serviceRapiddocsFilepath.File == nil || typeRapiddocsFilepath.File != nil {
		// If the service is a root client or the type is already defined
		// in a particular non-root package, we can leave it as-is.
		return false
	}
	if !packagePathIsEqual(typeRapiddocsFilepath, serviceRapiddocsFilepath) {
		// We only want to set the file info if the type is defined in the
		// same package as the service.
		return false
	}
	filename := serviceRapiddocsFilepath.File.SnakeCase.UnsafeName
	return !isReservedFilename(filename)
}

func packagePathIsEqual(a, b *rapiddocsir.RapiddocsFilepath) bool {
...
func fileInfoToErrors(
	rootPackageName string,
	irErrorDeclarations map[rapiddocsir.ErrorId]*rapiddocsir.ErrorDeclaration,
) map[fileInfo][]*rapiddocsir.ErrorDeclaration {
	result := make(map[fileInfo][]*rapiddocsir.ErrorDeclaration)
	for _, irErrorDeclaration := range irErrorDeclarations {
		var elements []string
		for _, packageName := range irErrorDeclaration.Name.RapiddocsFilepath.PackagePath {
...
func zeroValueForTypeReference(typeReference *rapiddocsir.TypeReference, types map[ir.TypeId]*ir.TypeDeclaration) string {
...
func zeroValueForLiteral(literal *rapiddocsir.Literal) string {
	switch literal.Type {
	case "string":
		return `""`
	case "boolean":
...
func zeroValueForPrimitive(primitive rapiddocsir.PrimitiveTypeV1) string {
	switch primitive {
	case rapiddocsir.PrimitiveTypeV1String, rapiddocsir.PrimitiveTypeV1BigInteger:
		return `""`
	case rapiddocsir.PrimitiveTypeV1Integer, rapiddocsir.PrimitiveTypeV1Long, rapiddocsir.PrimitiveTypeV1Uint, rapiddocsir.PrimitiveTypeV1Uint64, rapiddocsir.PrimitiveTypeV1Float, rapiddocsir.PrimitiveTypeV1Double:
		return "0"
	case rapiddocsir.PrimitiveTypeV1Boolean:
		return "false"
	case rapiddocsir.PrimitiveTypeV1DateTime, rapiddocsir.PrimitiveTypeV1Date:
		return "time.Time{}"
	case rapiddocsir.PrimitiveTypeV1Uuid:
		return "uuid.Nil"
	case rapiddocsir.PrimitiveTypeV1Base64:
		return "nil"
	}
	return "nil"
}
...
func getRootPackageName(ir *rapiddocsir.IntermediateRepresentation, packageNameOverride string) string {
...
func getExportedClientName(ir *rapiddocsir.IntermediateRepresentation, exportedClientNameOverride string, clientNameOverride string) string {
...
func needsPaginationHelpers(ir *rapiddocsir.IntermediateRepresentation) bool {
	for _, irService := range ir.Services {
		for _, irEndpoint := range irService.Endpoints {
...
func needsFileUploadHelpers(ir *rapiddocsir.IntermediateRepresentation) bool {
	for _, irService := range ir.Services {
		for _, irEndpoint := range irService.Endpoints {
