...
import { DocsDefinitionResolver, filterOssWorkspaces } from "@khulnasoft/docs-resolver";
import {
    APIV1Read,
    APIV1Write,
    DocsV1Read,
...
} from "@khulnasoft/fdr-sdk";
import { AbsoluteFilePath, convertToRapiddocsHostAbsoluteFilePath, relative } from "@khulnasoft/fs-utils";
...
export async function getPreviewDocsDefinition({
    ...
    const rapiddocsWorkspaces = await Promise.all(
        apiWorkspaces.map(
            async (workspace) =>
                await workspace.toRapiddocsWorkspace(
                    { context },
    ...
    const resolver = new DocsDefinitionResolver(
        ...
        async (files) =>
            files.map((file) => {
                ...
                filesV2[fileId] = {
                    type: "url",
                    url: RapiddocsNavigation.Url(`/_local${convertToRapiddocsHostAbsoluteFilePath(file.absoluteFilePath)}`)
                };
                return {
