...
export class PackageTreeGenerator {
    ...
    public addPackageRedirection({ from, to }: { from: RapiddocsFilepath; to: RapiddocsFilepath }): void {
        const package_ = this.getPackageForRapiddocsFilepath(from);
    ...
    public addDocs(rapiddocsFilepath: RapiddocsFilepath, docs: string): void {
        const package_ = this.getPackageForRapiddocsFilepath(rapiddocsFilepath);
    ...
    public addSubpackage(rapiddocsFilepath: RapiddocsFilepath): void {
        this.getPackageForRapiddocsFilepath(rapiddocsFilepath);
    }

    public addType(typeId: TypeId, type: TypeDeclaration): void {
        this.getPackageForRapiddocsFilepath(type.name.rapiddocsFilepath).types.push(typeId);
    }

    public addError(errorId: ErrorId, error: ErrorDeclaration): void {
        this.getPackageForRapiddocsFilepath(error.name.rapiddocsFilepath).errors.push(errorId);
    }

    public addService(serviceId: ServiceId, service: HttpService): void {
        const package_ = this.getPackageForRapiddocsFilepath(service.name.rapiddocsFilepath);
    ...
    public addWebhookGroup(webhookGroupId: WebhookGroupId, rapiddocsFilepath: RapiddocsFilepath): void {
        const package_ = this.getPackageForRapiddocsFilepath(rapiddocsFilepath);
    ...
    public addWebSocketChannel(websocketChannelId: WebSocketChannelId, rapiddocsFilepath: RapiddocsFilepath): void {
        const package_ = this.getPackageForRapiddocsFilepath(rapiddocsFilepath);
        if (package_.webhooks != null) {
    ...
    private getPackageForRapiddocsFilepath(
        rapiddocsFilepath: RapiddocsFilepath,
        index = 0,
        parent = this.rootPackage
    ): UnprocessedPackage {
        const nextPart = rapiddocsFilepath.allParts[index];
        if (nextPart == null) {
            return parent;
        }
        ...
        const rapiddocsFilepathForNextParent: RapiddocsFilepath = {
            allParts: rapiddocsFilepath.allParts.slice(0, nextIndex),
            packagePath: rapiddocsFilepath.packagePath.slice(0, nextIndex),
            file: nextIndex === rapiddocsFilepath.allParts.length ? rapiddocsFilepath.file : undefined
        };
        let nextParent = subpackagesInParent.find(
            (subpackage) => subpackage.name.originalName === nextPart.originalName
        );
        if (nextParent == null) {
            const newParentId = IdGenerator.generateSubpackageId(rapiddocsFilepathForNextParent);
            const newParent: UnprocessedSubpackage = {
                docs: undefined,
                rapiddocsFilepath: rapiddocsFilepathForNextParent,
                name: nextPart,
                service: undefined,
                types: [],
                errors: [],
        ...
        return this.getPackageForRapiddocsFilepath(rapiddocsFilepath, nextIndex, nextParent);
    }
}
