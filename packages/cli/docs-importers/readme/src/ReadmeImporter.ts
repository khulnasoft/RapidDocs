...
import { DocsImporter, RapiddocsDocsBuilder, RapiddocsDocsNavigationBuilder, TabInfo } from "@khulnasoft/docs-importer-commons";
import { DEFAULT_LAYOUT } from "@khulnasoft/docs-importer-commons";
...
export class ReadmeImporter extends DocsImporter<ReadmeImporter.Args> {
    ...
    public async import({ args, builder }: { args: ReadmeImporter.Args; builder: RapiddocsDocsBuilder }): Promise<void> {
        const urlObj = new URL(args.readmeUrl);
        const html = await fetchPageHtml({ url: urlObj });
        const absolutePathToOutput: AbsoluteFilePath = AbsoluteFilePath.of(join(process.cwd(), "rapiddocs"));
    ...
    }): Promise<RapiddocsDocsNavigationBuilder> {
    ...
    private getDefaultDocumentationTab(builder: RapiddocsDocsBuilder): TabInfo {
