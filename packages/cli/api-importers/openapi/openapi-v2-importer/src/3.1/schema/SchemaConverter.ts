...
import { DeclaredTypeName, Type, TypeDeclaration, TypeId } from "@khulnasoft/ir-sdk";

import { AbstractConverter } from "../../AbstractConverter";
import { ErrorCollector } from "../../ErrorCollector";
import { RapiddocsEnumExtension } from "../../extensions/x-rapiddocs-enum";
...
export class SchemaConverter extends AbstractConverter<OpenAPIConverterContext3_1, SchemaConverter.Output> {
    ...
    public async convert({
        ...
        if (this.schema.enum?.length) {
            const rapiddocsEnumConverter = new RapiddocsEnumExtension({
                breadcrumbs: this.breadcrumbs,
                schema: this.schema
            });
            const maybeRapiddocsEnum = rapiddocsEnumConverter.convert({ context, errorCollector });
