...
export function mergeEndpointPairs<EndpointType extends { method: string }>({
    ...
    const toRet: RapiddocsNavigation.V1.ApiPackageChild[] = [];

    const methodAndPathToEndpointNode = new Map<string, RapiddocsNavigation.V1.EndpointNode>();
    children.forEach((child) => {
        ...
        const pairNode: RapiddocsNavigation.V1.EndpointPairNode = {
            id: RapiddocsNavigation.V1.NodeId(`${apiDefinitionId}:${nonStream.endpointId}+${stream.endpointId}`),
            type: "endpointPair",
            stream,
            nonStream
        };
