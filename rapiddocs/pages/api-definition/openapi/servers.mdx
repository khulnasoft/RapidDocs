...
If you have more than one server, we recommend specifying an `x-rapiddocs-server-name` to name 
the server. 
...
servers: 
  - x-rapiddocs-server-name: Production
    url: https://api.yourcompany.com/
  - x-rapiddocs-server-name: Production_EU
    url: https://api.eu.yourcompany.com/
```
...
at difrapiddocst URLs. For example, your AI endpoints might be hosted at `ai.yourcompany.com` and the rest 
...
Once you've specified the environments in your `generators.yml`, you can use the `x-rapiddocs-server-name` 
...
<Info>To see an example of this in production, check out the Chariot [generators.yml](https://github.com/chariot-giving/chariot-openapi/blob/main/rapiddocs/generators.yml)</Info>