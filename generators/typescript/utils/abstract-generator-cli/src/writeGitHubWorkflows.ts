...
function constructWorkflowYaml({
    ...
    config: RapiddocsGeneratorExec.GeneratorConfig;
    publishInfo: RapiddocsGeneratorExec.NpmGithubPublishInfo | undefined;
    isPackagePrivate: boolean;
    publishToJsr: boolean;
}) {
...
function getTestJob({ config }: { config: RapiddocsGeneratorExec.GeneratorConfig }): string {
    ...
    //         run: |
    //           yarn
    //           yarn rapiddocs test --command='jest --env=node'
    //           yarn rapiddocs test --command='jest --env=jsdom'
    // `;
    //     } else {
    return `
  test:
