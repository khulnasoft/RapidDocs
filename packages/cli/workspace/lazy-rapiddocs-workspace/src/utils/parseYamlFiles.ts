...
import { RapiddocsFile, ParsedRapiddocsFile } from "@khulnasoft/api-workspace-commons";
...
export async function parseYamlFiles(files: readonly RapiddocsFile[]): Promise<Parser.Result> {
    const parsedFiles: Record<RelativeFilePath, ParsedRapiddocsFile<unknown>> = {};
    const failures: Record<RelativeFilePath, WorkspaceLoader.FileParseFailure> = {};

    function parseFilePath(file: RapiddocsFile) {
        try {
