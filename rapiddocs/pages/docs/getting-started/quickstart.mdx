---
title: Quickstart
subtitle: Start building beautiful documentation in under 5 minutes
---

<img alt="Docs cascade" src="./docs-cascade.png" />

With Rapiddocs, you can build beautiful developer documentation that matches your brand. Rapiddocs supports writing pages (written in Markdown) and generating API Reference documentation (from an OpenAPI Specification).

In this guide, we'll show you how to get started with Rapiddocs in under 5 minutes.

<Steps toc={true}>
  ### Initialize your `rapiddocs` folder
    
    All the configurations for your docs live in the `rapiddocs` folder. Inside, you'll 
    find a `docs.yml` file that contains all the settings for your documentation.

  <AccordionGroup toc={true}>
    <Accordion title='Clone the starter repository'>
      Get started by cloning the [starter template](https://github.com/khulnasoft/docs-starter).

      <CodeBlocks>
        ```bash title="SSH"
        git clone git@github.com:khulnasoft/docs-starter.git
        ```
        ```bash title="HTTPS"
        git clone https://github.com/khulnasoft/docs-starter.git
        ```
      </CodeBlocks>

      Next, please update the template settings to use your organization.

      <Warning title="Edit template settings">
        Please edit the details `rapiddocs.config.json` and `docs.yml` with your organization
        name.

        <CodeBlocks>
          <CodeBlock title="rapiddocs.config.json">
          ```json  {2}
          {
            "organization": "{{YOUR_ORGANIZATION}}",
            "version": "0.x.x"
          }
          ```
          </CodeBlock>
          <CodeBlock title="docs.yml">
          ```yml  {2}
          instances:
            - url: {{YOUR_ORGANIZATION}}.docs.buildwithrapiddocs.com
          ```
          </CodeBlock>
        </CodeBlocks>
      </Warning>

      Finally, run `rapiddocs generate --docs` to generate your documentation.
    </Accordion>
    <Accordion title='Use the Rapiddocs CLI'>
      If you prefer, you can use our CLI to create a new project. Install the CLI
      by running

      ```bash
      npm install -g khulnasoft
      ```

      Then run

      ```bash
      rapiddocs init --docs
      ```

      You will see a new `rapiddocs` folder in your project with the following structure:

      ```bash
        rapiddocs
        ├─ docs.yml
        └─ rapiddocs.config.json
      ```

      Finally, run `rapiddocs generate --docs` to generate your documentation.
    </Accordion>

  </AccordionGroup>

### Update your docs

<Note>
  We provide a white-glove migration service as part of our Enterprise plan. Interested? Request it
  [here](https://buildwithrapiddocs.com/contact).
</Note>

  <AccordionGroup toc={true}>
    <Accordion title='Add content' >
      Add content with MDX files. 
  
      ```markdown
      ---
      title: "Page Title"
      description: "Subtitle (optional)" 
      ---

      Hello world!
      ```

      <Note title="Supported Syntax">
      Rapiddocs supports [GitHub flavored Markdown (GFM)](https://github.github.com/gfm/) within MDX files, no plugin required.
      </Note>

      In order for the Markdown page to show up, you'll need to reference them from your `docs.yml` file. You
      can reference the Markdown page within a section or as a standalone page.

      ```yml
      navigation:
        - page: "Hello World"
          path: "pages/hello-world.mdx"
        - section: Overview
          content:
            - page: QuickStart
              path: pages/hello-world.mdx
      ```
    </Accordion>
    <Accordion title='Add an API Reference' >
      Add an API Reference by adding an OpenAPI Specification to your project.

      ```bash
      rapiddocs init --openapi /path/to/openapi.yml
      ```

      This will create an `openapi.yml` file in your project. You can reference this file in your
      `docs.yml` file by adding an api block.

      ```yml
      navigation:
        - api: "API Reference"
      ```
    </Accordion>
    <Accordion title='Brand your docs'>
      All of the branding for your docs can be configured in the `docs.yml` file.

      For example, to set the logos, colors, and fonts for your docs, you can
      add the following to your `docs.yml` file:

      <CodeBlock title="docs.yml">
      ```yml
      colors:
        accent-primary:
          dark: "#f0c193"
          light: "#af5f1b"

      logo:
        dark: ./docs/assets/logo-dark.svg
        light: ./docs/assets/logo-light.svg
        height: 40
        href: https://buildwithrapiddocs.com/

      favicon: ./docs/assets/favicon.png
      ```
      </CodeBlock>

    </Accordion>

  </AccordionGroup>
  
### Publish to production

  <AccordionGroup toc={true}>
    <Accordion title='Host on a custom domain'>
      Rapiddocs supports hosting your docs website on a custom domain or on a 
      custom subpath (e.g. https://domain.com/docs).

      Please reach out to the Rapiddocs team at support@buildwithrapiddocs.com to configure this.
    </Accordion>
    <Accordion title='Configure analytics'>
      Rapiddocs supports integrations with a variety of providers such as PostHog, Segment, Intercom,
      Google Tag Manager, etc.
      Find out more on this [page](/learn/docs/integrations/overview).
    </Accordion>

  </AccordionGroup>

</Steps>

Below are some examples of documentation websites that have been published using Rapiddocs:

<CardGroup cols={3}>
  <Card
    title="Hume"
    href="https://dev.hume.ai/intro"
    icon={<img src="https://www.hume.ai/icons/icon-144x144.png" alt="Hume logo" />}
  />
  <Card
    title="Primer"
    href="https://primer.io/docs/api"
    icon={
      <img
        src="https://assets-global.website-files.com/65bcc4714739c54deeb9bac7/65d7663e0ceca67e8b5709c5_favicon.png"
        alt="Primer logo"
      />
    }
  />
  <Card
    title="AssemblyAI"
    href="https://www.assemblyai.com/docs/api-reference"
    icon={<img src="https://www.assemblyai.com/favicon.png" alt="AssemblyAI logo" />}
  />
  <Card
    title="SuperAgent"
    href="https://docs.superagent.sh/"
    icon={<img src="https://framerusercontent.com/images/P75NaYul3IVsoMqx4RMALvngO4.png" alt="SuperAgent logo" />}
  />
  <Card
    title="MultiOn"
    href="https://docs.multion.ai/"
    icon={<img src="https://www.multion.ai/android-chrome-192x192.png" alt="MultiOn logo" />}
  />
  <Card
    title="Coactive"
    href="https://docs.coactive.ai/"
    icon={
      <img
        src="https://images.prismic.io/coactive/6d31b17f-cabb-492d-93ef-a9f42e03aaa2_coactive-logo-home.png?ixlib=gatsbyFP&auto=compress%2Cformat&fit=max"
        alt="Coactive logo"
      />
    }
  />
  <Card
    title="Zep"
    href="https://help.getzep.com/"
    icon={
      <img
        src="https://assets-global.website-files.com/660b1c076d13b7e2967a499d/660c6dc329d8b16a8468f5ba_Asset%2017.png"
        alt="Zep logo"
      />
    }
  />
  <Card
    title="Rightbrain"
    href="https://docs.rightbrain.ai/intro"
    icon={
      <img
        src="https://fdr-prod-docs-files-public.s3.amazonaws.com/https://rightbrain.docs.buildwithrapiddocs.com/2024-08-05T10:16:06.892Z/docs/assets/Logo.png"
        alt="Rightbrain logo"
      />
    }
  />
</CardGroup>
