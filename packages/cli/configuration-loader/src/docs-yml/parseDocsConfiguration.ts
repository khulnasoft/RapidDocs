...
import { AbsoluteFilePath, dirname, doesPathExist, listFiles, resolve } from "@khulnasoft/fs-utils";
...
async function getNavigationConfiguration({
    ...
    if (navigation != null) {
        ...
        for (const version of versions) {
            const absoluteFilepathToVersionFile = resolve(absolutePathToRapiddocsFolder, version.path);
    ...
    if (item.layout != null) {
        const layout = await Promise.all(
            item.layout.map((item) =>
                convertNavigationItem({ rawConfig: item, absolutePathToRapiddocsFolder, absolutePathToConfig, context })
            )
        );
        return {
