...
function testFixture(fixtureName: string) {
    ...
    describe(fixtureName, () => {
        it("Fails if API is not specified", async () => {
            ...
            const { stdout, failed } = await runRapiddocsCli(["ir", (await tmp.file()).path], {
                cwd: fixturePath,
                reject: false
            });
        ...
        it("Succeeds if API is specified", async () => {
            const fixturePath = path.join(FIXTURES_DIR, fixtureName);
            const { failed } = await runRapiddocsCli(["--api", "api1", "ir", (await tmp.file()).path], {
                cwd: fixturePath
            });
        ...
        it("Fail if API does not exist", async () => {
            const fixturePath = path.join(FIXTURES_DIR, fixtureName);
            const { failed } = await runRapiddocsCli(["--api", "api3", "ir", (await tmp.file()).path], {
                cwd: fixturePath,
                reject: false
            });
            expect(failed).toBe(true);
