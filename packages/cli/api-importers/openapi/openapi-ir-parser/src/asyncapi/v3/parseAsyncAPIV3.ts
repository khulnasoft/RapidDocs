...
import { RapiddocsOpenAPIExtension } from "../..";
...
export function parseAsyncAPIV3({
    ...
    for (const [operationId, operation] of Object.entries(document.operations ?? {})) {
        if (getExtension<boolean>(operation, RapiddocsAsyncAPIExtension.IGNORE)) {
            continue;
        }
    ...
    for (const [channelPath, channel] of Object.entries(document.channels ?? {})) {
        if (getExtension<boolean>(channel, RapiddocsAsyncAPIExtension.IGNORE)) {
            continue;
        }
        ...
        if (channel.parameters != null) {
            for (const [name, parameter] of Object.entries(channel.parameters)) {
                ...
                const isOptional = getExtension<boolean>(parameter, RapiddocsAsyncAPIExtension.RAPIDDOCS_PARAMETER_OPTIONAL);
                const parameterName = upperFirst(camelCase(channelPath)) + upperFirst(camelCase(name));
                const rapiddocsEnum = getExtension<RapiddocsEnumConfig>(parameter, RapiddocsAsyncAPIExtension.RAPIDDOCS_ENUM);
