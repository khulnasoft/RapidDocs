...
export declare namespace RapiddocsPaginationExtension {
...
export class RapiddocsPaginationExtension extends AbstractExtension<
    OpenAPIConverterContext3_1,
    RapiddocsPaginationExtension.Output
> {
    private readonly operation: object;
    private readonly document: OpenAPIV3.Document;
    public readonly key = "x-rapiddocs-pagination";

    constructor({ breadcrumbs, operation, document }: RapiddocsPaginationExtension.Args) {
        super({ breadcrumbs });
    ...
    public convert({
    ...
    }): RapiddocsPaginationExtension.Output | undefined {
        ...
        if (!result.success) {
            errorCollector.collect({
                message: `Invalid x-rapiddocs-pagination extension: ${result.error.message}`,
        ...
        if (typeof result.data === "boolean") {
            ...
            if (typeof topLevelPagination === "boolean") {
                errorCollector.collect({
                    message:
                        "Global pagination extension is a boolean, expected an object. Only endpoints may declare a boolean for x-rapiddocs-pagination.",
                    path: this.breadcrumbs
                });
                return undefined;
            }
