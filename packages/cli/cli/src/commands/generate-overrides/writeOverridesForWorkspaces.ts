...
import { RelativeFilePath, dirname, join } from "@khulnasoft/fs-utils";
import { OSSWorkspace, OpenAPILoader, getAllOpenAPISpecs } from "@khulnasoft/lazy-rapiddocs-workspace";
import { Schema } from "@khulnasoft/openapi-ir";
import { parse } from "@khulnasoft/openapi-ir-parser";
import { getEndpointLocation } from "@khulnasoft/openapi-ir-to-rapiddocs";
        ...
        project.apiWorkspaces.map(async (workspace) => {
            await cliContext.runTaskForWorkspace(workspace, async (context) => {
                if (workspace instanceof OSSWorkspace) {
                    ...
                    context.logger.warn("Skipping rapiddocs workspace definition generation");
                }
            });
        })
    );
        ...
        for (const endpoint of ir.endpoints) {
            ...
            if (pathItem != null && pathItem[endpoint.method] == null) {
                ...
                if (groupName.length > 0) {
                    sdkMethodNameExtensions["x-rapiddocs-sdk-group-name"] = groupName;
                }
                sdkMethodNameExtensions["x-rapiddocs-sdk-method-name"] = endpointLocation.endpointId;
...
function writeModels(existingSchemas: Record<string, Record<string, unknown>>, schemas: Record<string, Schema>) {
    for (const [schemaId, schema] of Object.entries(schemas)) {
        ...
        typeNameOverride["x-rapiddocs-type-name"] = "nameOverride" in schema ? (schema.nameOverride ?? schemaId) : schemaId;
