...
import { LOG_LEVELS, LogLevel, createLogger } from "@khulnasoft/logger";
import { getPosthogManager } from "@khulnasoft/posthog-manager";
import { Project } from "@khulnasoft/project-loader";
import { isVersionAhead } from "@khulnasoft/semver-utils";
import { RapiddocsCliError, Finishable, PosthogEvent, Startable, TaskContext, TaskResult } from "@khulnasoft/task-context";
import { Workspace } from "@khulnasoft/workspace-loader";
...
import { getRapiddocsUpgradeMessage } from "./upgrade-utils/getRapiddocsUpgradeMessage";
import { RapiddocsGeneratorUpgradeInfo, getProjectGeneratorUpgrades } from "./upgrade-utils/getGeneratorVersions";
...
export interface RapiddocsCliUpgradeInfo {
    isUpgradeAvailable: boolean;
    latestVersion: string;
}

export interface RapiddocsUpgradeInfo {
    cliUpgradeInfo: RapiddocsCliUpgradeInfo | undefined;
...
export class CliContext {
    ...
    private async nudgeUpgradeIfAvailable() {
        try {
            const upgradeInfo = await Promise.race<[Promise<RapiddocsUpgradeInfo>, Promise<never>]>([
                this.isUpgradeAvailable(),
                new Promise((_resolve, reject) => setTimeout(() => reject("Request timed out"), 300))
            ]);

            let upgradeMessage = await getRapiddocsUpgradeMessage({
    ...
    private readonly USE_NODE_18_OR_ABOVE_MESSAGE = "The Rapiddocs CLI requires Node 18+ or above.";
    ...
    private _isUpgradeAvailable: RapiddocsUpgradeInfo | undefined;
    public async isUpgradeAvailable({
        includePreReleases = false
    }: {
        includePreReleases?: boolean;
    } = {}): Promise<RapiddocsUpgradeInfo> {
        if (this._isUpgradeAvailable == null) {
            ...
            const cliUpgrade: RapiddocsCliUpgradeInfo = {
                isUpgradeAvailable,
                latestVersion: latestPackageVersion
            };

            // Check if the generators are upgradable
            const generatorUpgrades: RapiddocsGeneratorUpgradeInfo[] = await getProjectGeneratorUpgrades({
