...
import { getRapiddocsEncoding } from "../openapi/v3/extensions/getRapiddocsEncoding";
import { getRapiddocsEnum } from "../openapi/v3/extensions/getRapiddocsEnum";
import { getRapiddocsTypeExtension } from "../openapi/v3/extensions/getRapiddocsTypeExtension";
...
export function convertSchemaObject(
    ...
    const nameOverride =
        getExtension<string>(schema, RapiddocsOpenAPIExtension.TYPE_NAME) ??
        (context.options.useTitlesAsName ? getTitleAsName(schema.title) : undefined);
    const mixedGroupName =
        getExtension(schema, RapiddocsOpenAPIExtension.SDK_GROUP_NAME) ??
    ...
    const rapiddocsSchema = getRapiddocsTypeExtension({ schema, description, title, nameOverride, generatedName, availability });
    if (rapiddocsSchema != null) {
        return rapiddocsSchema;
    }
    ...
    if (schema.enum != null && (schema.type === "string" || schema.type == null)) {
        ...
        const rapiddocsEnum = getRapiddocsEnum(schema);

        if (schema.enum.length === 1 && schema.enum[0] != null && rapiddocsEnum == null) {
    ...
    if (schema.type === "boolean") {
        const literalValue = getExtension<boolean>(schema, RapiddocsOpenAPIExtension.BOOLEAN_LITERAL);
    ...
    if (schema.oneOf != null && schema.oneOf.length > 0) {
        const isUndiscriminated = getExtension(schema, RapiddocsOpenAPIExtension.IS_UNDISCRIMINATED);
        if (
...
export function convertToReferencedSchema(
    ...
    const nameOverride = getExtension<string>(schema, RapiddocsOpenAPIExtension.TYPE_NAME);
...
function getEncoding({
    ...
    const encoding = getRapiddocsEncoding({ schema, logger });
    if (encoding == null) {
        return undefined;
    }
