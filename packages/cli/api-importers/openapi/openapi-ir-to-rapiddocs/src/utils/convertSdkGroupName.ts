...
import { EndpointSdkName, SdkGroupName } from "@khulnasoft/openapi-ir";
...
export function convertSdkGroupNameToFileWithoutExtension(groupName: SdkGroupName | undefined): string {
    ...
    if (cleanedGroupName == null || cleanedGroupName.length === 0) {
        return RAPIDDOCS_PACKAGE_MARKER_FILENAME_NO_EXTENSION;
    }
            ...
            switch (group.type) {
                case "namespace": {
                    if (index < cleanedGroupName.length - 1) {
                        ...
                        fileNames.push(...[group.name, RAPIDDOCS_PACKAGE_MARKER_FILENAME_NO_EXTENSION]);
                    }
                    break;
                }
                default:
