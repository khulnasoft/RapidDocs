...
import { getRapiddocsBasePath } from "./extensions/getRapiddocsBasePath";
import { getRapiddocsGroups } from "./extensions/getRapiddocsGroups";
import { getRapiddocsVersion } from "./extensions/getRapiddocsVersion";
...
export function generateIr({
        ...
        Object.entries(openApi.components?.schemas ?? {})
            .map(([key, schema]) => {
                if (!isReferenceObject(schema)) {
                    const ignoreSchema = getExtension<boolean>(schema, RapiddocsOpenAPIExtension.IGNORE);
                    ...
                    if (ERROR_NAMES.has(key)) {
                        return [
                            ...
                            convertSchema(
                                ...
                                { ...schema, "x-rapiddocs-type-name": `${key}Body` } as any as OpenAPIV3.SchemaObject,
                                false,
                                context,
                                [key],
                                source,
    ...
    // Remove discriminants from discriminated unions since Rapiddocs handles this in the IR.
    ...
    const endpoints = endpointsWithExample.map((endpointWithExample): Endpoint => {
        // if x-rapiddocs-examples is not present, generate an example
    ...
    const groupInfo = getRapiddocsGroups({ document: openApi, context });

    const ir: OpenApiIntermediateRepresentation = {
        apiVersion: getRapiddocsVersion({
            context,
            document: openApi
        }),
        basePath: getRapiddocsBasePath(openApi),
