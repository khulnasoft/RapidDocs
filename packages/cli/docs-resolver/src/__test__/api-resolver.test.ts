...
import { docsYml, parseDocsConfiguration } from "@khulnasoft/configuration-loader";
...
it.skip("converts to api reference node", async () => {
    const docsWorkspace = await loadDocsWorkspace({
        rapiddocsDirectory: resolve(AbsoluteFilePath.of(__dirname), "fixtures/api-resolver/rapiddocs"),
        context
    });
    ...
    const result = await loadAPIWorkspace({
        absolutePathToWorkspace: resolve(AbsoluteFilePath.of(__dirname), "fixtures/api-resolver/rapiddocs"),
        context,
        cliVersion: "0.0.0",
