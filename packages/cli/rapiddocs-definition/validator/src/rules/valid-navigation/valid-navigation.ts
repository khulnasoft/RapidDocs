...
import { getAllDefinitionFiles, getAllNamedDefinitionFiles } from "@khulnasoft/api-workspace-commons";
import { RAPIDDOCS_PACKAGE_MARKER_FILENAME } from "@khulnasoft/configuration-loader";
import { keys } from "@khulnasoft/core-utils";
import { RelativeFilePath, dirname, join, relative } from "@khulnasoft/fs-utils";
            ...
            packageMarker: {
                navigation: (navigation, { relativeFilepath }) => {
                    ...
                    for (const actualItem of navigation) {
                        if (actualItem === RAPIDDOCS_PACKAGE_MARKER_FILENAME) {
                            violations.push({
                                severity: "fatal",
                                message: `${RAPIDDOCS_PACKAGE_MARKER_FILENAME} cannot be specified in navigation.`
                            });
