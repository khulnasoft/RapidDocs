...
import { Endpoint, HttpMethod } from "@khulnasoft/openapi-ir";
...
function getUnresolvedEndpointLocation(endpoint: Endpoint): EndpointLocation {
    ...
    if (operationId == null) {
        ...
        if (endpoint.summary != null) {
            return {
                file: RelativeFilePath.of(RAPIDDOCS_PACKAGE_MARKER_FILENAME),
...
export function getEndpointLocation(endpoint: Endpoint): EndpointLocation {
    ...
    if (endpoint.sdkName != null) {
        ...
        // only if the tag lines up with `x-rapiddocs-sdk-group-name` do we use it
