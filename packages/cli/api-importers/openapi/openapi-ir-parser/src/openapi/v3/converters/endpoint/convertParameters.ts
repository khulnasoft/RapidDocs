...
export function convertParameters({
    ...
    for (const parameter of parameters) {
        const shouldIgnore = getExtension<boolean>(parameter, RapiddocsOpenAPIExtension.IGNORE);
        if (shouldIgnore != null && shouldIgnore) {
            context.logger.debug(
                `${httpMethod.toUpperCase()} ${path} has a parameter marked with x-rapiddocs-ignore. Skipping.`
            );
            continue;
        }
