...
import { recursivelyVisitRawTypeReference } from "@khulnasoft/rapiddocs-definition-schema";
import {
    Availability,
    LiteralSchemaValue,
...
export function getRapiddocsTypeExtension({
    nameOverride,
    generatedName,
    title,
    schema,
    ...
    const groupName = getExtension(schema, RapiddocsOpenAPIExtension.SDK_GROUP_NAME);
    const rapiddocsType = getExtension<string>(schema, RapiddocsOpenAPIExtension.TYPE_DEFINITION);
    if (rapiddocsType == null) {
        return;
    }
    return getSchemaFromRapiddocsType({
        rapiddocsType,
        nameOverride,
        generatedName,
        title,
...
export function getSchemaFromRapiddocsType({
    rapiddocsType,
    nameOverride,
    generatedName,
    title,
