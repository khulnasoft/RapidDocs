...
FastAPI plays very nicely with Rapiddocs because it has the power to output OpenAPI Specifications! Below we'll outline some tips for generating a rich OpenAPI with FastAPI.
...
Rapiddocs will automatically generate clients that point to the servers you configure within your OpenAPI Specification, so it's important to specify the servers that your API will be hosted on.
...
Below, we've annotated a "good" route within FastAPI that has it's typings as well as Rapiddocs extensions to assist in naming.
...
@router.post(
    ...
    openapi_extra={                              #  <-- finally, you can add in your Rapiddocs extensions here, these extensions will produce SDK code that looks something like: `client.endpoints.create(...)` in python
        "x-rapiddocs-sdk-method-name": "create",
        "x-rapiddocs-sdk-group-name": "endpoints",
        "x-rapiddocs-availability": "beta",
    },
)
...
FastAPI allows you to specify examples for your data models, which Rapiddocs will pick up and use within your generated SDKs and documentation automatically.

For more information on leveraging examples within Rapiddocs, please refer to the [Rapiddocs documentation](/learn/api-definition/openapi/extensions/others#request--response-examples).
