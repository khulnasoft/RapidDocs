    ...
    migrateBackwards: (v7): IrVersions.V6.ir.IntermediateRepresentation => {
        return {
            ...
            services: v7.services.map((service) => {
                const lastRapiddocsFilepathPart = service.name.rapiddocsFilepath[service.name.rapiddocsFilepath.length - 1];

                const originalName =
                    lastRapiddocsFilepathPart != null ? `${lastRapiddocsFilepathPart.pascalCase.safeName}Service` : "Service";
                const camelCase =
                    lastRapiddocsFilepathPart != null ? `${lastRapiddocsFilepathPart.camelCase.unsafeName}Service` : "Service";
                const snakeCase =
                    lastRapiddocsFilepathPart != null ? `${lastRapiddocsFilepathPart.snakeCase.unsafeName}_service` : "service";
