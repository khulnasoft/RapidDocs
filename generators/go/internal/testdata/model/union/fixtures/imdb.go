...
import (
	json "encoding/json"
	fmt "fmt"
	internal "github.com/khulnasoft/rapiddocs-go/internal/testdata/model/union/fixtures/internal"
	time "time"
)
...
func NewUnionWithLiteralWithRapiddocs() *UnionWithLiteral {
	return &UnionWithLiteral{Type: "rapiddocs", rapiddocs: "rapiddocs"}
}
...
func (u *UnionWithLiteral) Rapiddocs() string {
	if u == nil {
		return ""
	}
	return u.rapiddocs
...
func (u *UnionWithLiteral) UnmarshalJSON(data []byte) error {
	...
	case "rapiddocs":
		var valueUnmarshaler struct {
			Rapiddocs string `json:"value,omitempty"`
		}
		if err := json.Unmarshal(data, &valueUnmarshaler); err != nil {
			return err
		}
		if valueUnmarshaler.Rapiddocs != "rapiddocs" {
			return fmt.Errorf("unexpected value for literal on type %T; expected %v got %v", u, "rapiddocs", valueUnmarshaler.Rapiddocs)
		}
		u.rapiddocs = valueUnmarshaler.Rapiddocs
	}
	return nil
}
...
func (u *UnionWithLiteral) Accept(visitor UnionWithLiteralVisitor) error {
	...
	default:
		return fmt.Errorf("invalid type %s in %T", u.Type, u)
	case "rapiddocs":
		return visitor.VisitRapiddocs(u.rapiddocs)
	}
}
...
func (u *UnionWithLiteral) validate() error {
	...
	var fields []string
	if u.rapiddocs != "" {
		fields = append(fields, "rapiddocs")
	}
	if len(fields) == 0 {
		if u.Type != "" {
			return fmt.Errorf("type %T defines a discriminant set to %q but the field is not set", u, u.Type)
