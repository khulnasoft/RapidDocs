...
class RapiddocsAwarePydanticModel:
    """
    A pydantic model generator that is aware of the Rapiddocs type representations.
    ...
    def _get_validators_generator(self) -> ValidatorsGenerator:
        ...
        if v1_root_type is not None and self._custom_config.version in (
            ...
            unique_name = []
            if self._type_name is not None:
                unique_name = [path.snake_case.unsafe_name for path in self._type_name.rapiddocs_filepath.package_path]
    ...
    def __enter__(self) -> RapiddocsAwarePydanticModel:
        return self
