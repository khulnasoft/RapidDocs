...
import { ts } from "@khulnasoft/typescript-ast";
...
export class DynamicTypeLiteralMapper {
    ...
    private convertList({ list, value }: { list: RapiddocsIr.dynamic.TypeReference; value: unknown }): ts.TypeLiteral {
    ...
    private convertSet({ set, value }: { set: RapiddocsIr.dynamic.TypeReference; value: unknown }): ts.TypeLiteral {
        if (!Array.isArray(value)) {
            this.context.errors.add({
                severity: Severity.Critical,
                message: `Expected array but got: ${typeof value}`
    ...
    private convertMap({ map, value }: { map: RapiddocsIr.dynamic.MapType; value: unknown }): ts.TypeLiteral {
    ...
    private convertObject({ object_, value }: { object_: RapiddocsIr.dynamic.ObjectType; value: unknown }): ts.TypeLiteral {
        const properties = this.context.associateByWireValue({
    ...
    private convertEnum({ enum_, value }: { enum_: RapiddocsIr.dynamic.EnumType; value: unknown }): ts.TypeLiteral {
    ...
    private getEnumValue({ enum_, value }: { enum_: RapiddocsIr.dynamic.EnumType; value: unknown }): string | undefined {
