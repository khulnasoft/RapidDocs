...
import { DEFINITION_DIRECTORY, GeneratorGroup, GeneratorInvocation } from "@khulnasoft/configuration";
...
export async function runWithCustomFixture({
    ...
    const apiWorkspace = await convertGeneratorWorkspaceToRapiddocsWorkspace({
    ...
    try {
        const rapiddocsWorkspace: RapiddocsWorkspace = await apiWorkspace.toRapiddocsWorkspace(
            { context: taskContext },
        ...
        await dockerGeneratorRunner.runGeneratorFromGroup({
            rapiddocsWorkspace,
            absolutePathToRapiddocsDefinition: join(pathToFixture, RelativeFilePath.of(DEFINITION_DIRECTORY)),
            taskContext,
        ...
        await writeDotMock({
            ...
            absolutePathToRapiddocsDefinition: pathToFixture
        });
