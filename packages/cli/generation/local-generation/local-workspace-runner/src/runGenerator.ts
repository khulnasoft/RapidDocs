...
import { runDocker } from "@khulnasoft/docker-utils";
import { AbsoluteFilePath, streamObjectToFile, waitUntilPathExists } from "@khulnasoft/fs-utils";
import { ApiDefinitionSource, IntermediateRepresentation, SourceConfig } from "@khulnasoft/ir-sdk";
import { TaskContext } from "@khulnasoft/task-context";
import { RapiddocsWorkspace, IdentifiableSource } from "@khulnasoft/workspace-loader";

import { RapiddocsGeneratorExec } from "@rapiddocs-rapiddocs/generator-exec-sdk";
import * as RapiddocsGeneratorExecParsing from "@rapiddocs-rapiddocs/generator-exec-sdk/serialization";
...
export async function runGenerator({
    ...
    const parsedConfig = await RapiddocsGeneratorExecParsing.GeneratorConfig.json(config);
    if (!parsedConfig.ok) {
...
function getSourceConfig(workspace: RapiddocsWorkspace): SourceConfig {
    return {
