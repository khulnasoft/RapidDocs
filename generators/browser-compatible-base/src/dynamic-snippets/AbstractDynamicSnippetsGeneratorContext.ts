import { assertNever, keys } from "@khulnasoft/core-utils";
...
import { RapiddocsGeneratorExec } from "../GeneratorNotificationService";
...
export abstract class AbstractDynamicSnippetsGeneratorContext {
    public config: RapiddocsGeneratorExec.GeneratorConfig;
    public errors: ErrorReporter;

    private _ir: RapiddocsIr.dynamic.DynamicIntermediateRepresentation;
    ...
    public getSingleFileValue({
        ...
        property: RapiddocsIr.dynamic.FileUploadRequestBodyProperty.File_;
    ...
    public getFileArrayValues({
        ...
        property: RapiddocsIr.dynamic.FileUploadRequestBodyProperty.FileArray;
        record: Record<string, unknown>;
    }): string[] | undefined {
    ...
    public resolveNamedType({ typeId }: { typeId: RapiddocsIr.TypeId }): RapiddocsIr.dynamic.NamedType | undefined {
    ...
    public resolveDiscriminatedUnionTypeInstance({
        ...
        discriminatedUnion: RapiddocsIr.dynamic.DiscriminatedUnionType;
        value: unknown;
    ...
    public resolveEndpointOrThrow(rawEndpoint: string): RapiddocsIr.dynamic.Endpoint[] {
    ...
    public resolveEndpointLocationOrThrow(location: RapiddocsIr.dynamic.EndpointLocation): RapiddocsIr.dynamic.Endpoint[] {
        const endpoints: RapiddocsIr.dynamic.Endpoint[] = [];
    ...
    ): body is RapiddocsIr.dynamic.InlinedRequestBody.FileUpload {
        switch (body.type) {
            case "fileUpload":
                return true;
            case "properties":
    ...
    public resolveEnvironmentName(environmentID: string): RapiddocsIr.Name | undefined {
    ...
    public isSingleEnvironmentID(environment: RapiddocsIr.dynamic.EnvironmentValues): environment is RapiddocsIr.EnvironmentId {
        return typeof environment === "string";
    }

    public isMultiEnvironmentValues(
        environment: RapiddocsIr.dynamic.EnvironmentValues
    ): environment is RapiddocsIr.dynamic.MultipleEnvironmentUrlValues {
        return typeof environment === "object";
    }

    public validateMultiEnvironmentUrlValues(
        multiEnvironmentUrlValues: RapiddocsIr.dynamic.MultipleEnvironmentUrlValues
    ): boolean {
        if (this._ir.environments == null) {
            this.errors.add({
                severity: Severity.Critical,
    ...
    public isOptional(typeReference: RapiddocsIr.dynamic.TypeReference): boolean {
    ...
    public isNullable(typeReference: RapiddocsIr.dynamic.TypeReference): boolean {
        switch (typeReference.type) {
            case "nullable":
                return true;
            case "optional":
    ...
    private includeFileUploadBodyInWrappedRequest({
        ...
        fileUpload: RapiddocsIr.dynamic.FileUploadRequestBody;
        inlineFileProperties: boolean;
    }): boolean {
        return (
    ...
    private fileUploadHasBodyProperties({ fileUpload }: { fileUpload: RapiddocsIr.dynamic.FileUploadRequestBody }): boolean {
    ...
    private fileUploadHasFileProperties({ fileUpload }: { fileUpload: RapiddocsIr.dynamic.FileUploadRequestBody }): boolean {
        return fileUpload.properties.some((property) => {
            switch (property.type) {
                case "file":
                case "fileArray":
    ...
    private isListTypeReference(typeReference: RapiddocsIr.dynamic.TypeReference): boolean {
