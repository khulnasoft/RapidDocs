import { getTextOfTsNode } from "@rapiddocs-typescript/commons";
import { SdkContext } from "@rapiddocs-typescript/contexts";
import { Code, code } from "ts-poet";

import { AbstractReadmeSnippetBuilder } from "@khulnasoft/base-generator";
...
import { EndpointId, FeatureId, RapiddocsFilepath, HttpEndpoint, SdkRequestWrapper } from "@rapiddocs-rapiddocs/ir-sdk/api";
...
export class ReadmeSnippetBuilder extends AbstractReadmeSnippetBuilder {
    private static ABORTING_REQUESTS_FEATURE_ID: RapiddocsGeneratorCli.FeatureId = "ABORTING_REQUESTS";
    private static EXCEPTION_HANDLING_FEATURE_ID: RapiddocsGeneratorCli.FeatureId = "EXCEPTION_HANDLING";
    private static REQUEST_AND_RESPONSE_TYPES_FEATURE_ID: RapiddocsGeneratorCli.FeatureId = "REQUEST_AND_RESPONSE_TYPES";
    private static RUNTIME_COMPATIBILITY_FEATURE_ID: RapiddocsGeneratorCli.FeatureId = "RUNTIME_COMPATIBILITY";
    private static STREAMING_FEATURE_ID: RapiddocsGeneratorCli.FeatureId = "STREAMING";
    private static PAGINATION_FEATURE_ID: RapiddocsGeneratorCli.FeatureId = "PAGINATION";
    private static RAW_RESPONSES_FEATURE_ID: RapiddocsGeneratorCli.FeatureId = "RAW_RESPONSES";
    private static ADDITIONAL_HEADERS_FEATURE_ID: RapiddocsGeneratorCli.FeatureId = "ADDITIONAL_HEADERS";
    ...
    public buildReadmeSnippets(): Record<RapiddocsGeneratorCli.FeatureId, string[]> {
        const snippets: Record<RapiddocsGeneratorCli.FeatureId, string[]> = {};
        snippets[RapiddocsGeneratorCli.StructuredFeatureId.Usage] = this.buildUsageSnippets();
        snippets[RapiddocsGeneratorCli.StructuredFeatureId.Retries] = this.buildRetrySnippets();
        snippets[RapiddocsGeneratorCli.StructuredFeatureId.Timeouts] = this.buildTimeoutSnippets();
        ...
        if (this.isPaginationEnabled) {
            snippets[RapiddocsGeneratorCli.StructuredFeatureId.Pagination] = this.buildPaginationSnippets();
        }
    ...
    private buildStreamingSnippets(): string[] {
        const explicitlyConfigured = this.getExplicitlyConfiguredSnippets(
            RapiddocsGeneratorCli.StructuredFeatureId.Streaming
        );
    ...
    private buildPaginationSnippets(): string[] {
        const explicitlyConfigured = this.getExplicitlyConfiguredSnippets(
            RapiddocsGeneratorCli.StructuredFeatureId.Pagination
        );
        if (explicitlyConfigured != null) {
            return explicitlyConfigured;
        }
    ...
    private buildUsageSnippets(): string[] {
        const explicitlyConfigured = this.getExplicitlyConfiguredSnippets(RapiddocsGeneratorCli.StructuredFeatureId.Usage);
        if (explicitlyConfigured != null) {
            return explicitlyConfigured;
        }
...
`
    ...
    private buildRetrySnippets(): string[] {
        const retryEndpoints = this.getEndpointsForFeature(RapiddocsGeneratorCli.StructuredFeatureId.Retries);
...
`
    ...
    private buildTimeoutSnippets(): string[] {
        const timeoutEndpoints = this.getEndpointsForFeature(RapiddocsGeneratorCli.StructuredFeatureId.Timeouts);
...
`
        ...
        for (const service of Object.values(this.context.ir.services)) {
            for (const endpoint of service.endpoints) {
                endpoints[endpoint.id] = {
                    endpoint,
                    rapiddocsFilepath: service.name.rapiddocsFilepath
                };
            }
        }
        return endpoints;
    ...
    private buildSnippets(endpointSnippets: RapiddocsGeneratorExec.Endpoint[]): Record<EndpointId, string> {
    ...
    private getEndpointSnippetString(endpoint: RapiddocsGeneratorExec.Endpoint): string {
    ...
    private getMethodCall(endpoint: EndpointWithFilepath): string {
        return `${this.getAccessFromRootClient(endpoint.rapiddocsFilepath)}.${this.getEndpointMethodName(
            endpoint.endpoint
        )}`;
    }

    private getAccessFromRootClient(rapiddocsFilepath: RapiddocsFilepath): string {
        const clientAccessParts = rapiddocsFilepath.allParts.map((part) => part.camelCase.unsafeName);
