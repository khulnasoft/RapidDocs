...
from rapiddocs.generator_exec import GeneratorConfig, PypiMetadata
from rapiddocs.generator_exec.config import (
...
from rapiddocs_python.codegen.project import Project, ProjectConfig
from rapiddocs_python.external_dependencies.ruff import RUFF_DEPENDENCY
    ...
    def generate_project(
        ...
        *,
        ...
        if output_mode_union.type == "downloadFiles":
            ...
            # we run ruff using the rapiddocs_python poetry.toml (copied into the docker)
            publisher._run_command(
                command=["poetry", "run", "ruff", "format", "/rapiddocs/output"],
                safe_command="poetry run ruff format /rapiddocs/output",
                cwd="/",
            )
...
"""
      ...
      - name: Test
        run: rapiddocs test --command "poetry run pytest -rP ."
"""
        else:
            workflow_yaml += """
      - name: Test
