...
import { DocsDefinitionResolver, convertIrToApiDefinition } from "@khulnasoft/docs-resolver";
import { APIV1Read, ApiDefinition, RapiddocsNavigation } from "@khulnasoft/fdr-sdk";
import { AbsoluteFilePath, RelativeFilePath, join, relative } from "@khulnasoft/fs-utils";
...
export const ValidMarkdownLinks: Rule = {
    ...
    create: async ({ workspace, rapiddocsWorkspaces, ossWorkspaces }) => {
        ...
        const root = RapiddocsNavigation.migrate.RapiddocsNavigationV1ToLatest.create().root(resolvedDocsDefinition.config.root);
        ...
        const endpoints: RapiddocsNavigation.NavigationNodeApiLeaf[] = [];
        collector.slugMap.forEach((node, slug) => {
            ...
            if (RapiddocsNavigation.isApiLeaf(node)) {
                endpoints.push(node);
            }
        ...
        return {
            ...
            apiSection: async ({ workspace: apiWorkspace, config }) => {
                const rapiddocsWorkspace = await apiWorkspace.toRapiddocsWorkspace(
                ...
                const ir = generateIntermediateRepresentation({
                    ...
                    context: NOOP_CONTEXT,
                    sourceResolver: new SourceResolverImpl(NOOP_CONTEXT, rapiddocsWorkspace)
                });
