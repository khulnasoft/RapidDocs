...
import { EndpointExample } from "@khulnasoft/openapi-ir";
...
export function getExamplesFromExtension(
    ...
    const exampleEndpointCalls = getExtension<RawSchemas.ExampleEndpointCallSchema[]>(
        operationObject,
        RapiddocsOpenAPIExtension.EXAMPLES
    );

    const validatedExampleEndpointCalls: RawSchemas.ExampleEndpointCallArraySchema = (
        exampleEndpointCalls ?? []
    ).filter((example) => {
        const maybeRapiddocsExample = RawSchemas.serialization.ExampleEndpointCallSchema.parse(example);
        if (!maybeRapiddocsExample.ok) {
            context.logger.error(
                `Failed to parse x-rapiddocs-example in ${operationContext.path}/${operationContext.method}`
            );
        }
        return maybeRapiddocsExample.ok;
    });
