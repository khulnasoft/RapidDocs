...
import { AsIsManager } from "@rapiddocs-typescript/commons/src/asIs/AsIsManager";
...
import { FdrSnippetTemplate, FdrSnippetTemplateClient, FdrSnippetTemplateEnvironment } from "@rapiddocs-rapiddocs/snippet-sdk";
...
export declare namespace SdkGenerator {
    export interface Init {
        ...
        rawConfig: RapiddocsGeneratorExec.GeneratorConfig;
        config: Config;
    }
...
export class SdkGenerator {
    ...
    private rawConfig: RapiddocsGeneratorExec.GeneratorConfig;
                ...
                ? new FdrSnippetTemplateClient({
                      environment:
                          this.config.executionEnvironment === "dev"
                              ? "https://registry-dev2.buildwithrapiddocs.com"
    ...
    private generateSnippets() {
        ...
        this.forEachService((service, packageId) => {
            ...
            let serviceReference = this.referenceConfigBuilder.addSection({
                title:
                    service.displayName ??
                    service.name.rapiddocsFilepath.allParts.map((part) => part.pascalCase.unsafeName).join(" ")
            });
            ...
            for (const endpoint of service.endpoints) {
                ...
                if (this.config.snippetTemplateFilepath != null && this.npmPackage != null) {
                    ...
                    if (snippetTemplate != null) {
                        const endpointPath = RapiddocsGeneratorExec.EndpointPath(getFullPathForEndpoint(endpoint));
                        this.context.logger.debug(
                ...
                for (const [index, example] of examplesForEndpoint.entries()) {
                    ...
                    if (snippet != null) {
                        const endpointSnippet: RapiddocsGeneratorExec.Endpoint = {
                            id: {
                                path: RapiddocsGeneratorExec.EndpointPath(getFullPathForEndpoint(endpoint)),
                                method: endpoint.method,
                                identifierOverride: endpoint.id
                            },
                            snippet: RapiddocsGeneratorExec.EndpointSnippet.typescript({
                                client: snippet
                            })
                        };
                    ...
                    if (index === 0 && serviceReference != null) {
                        ...
                        const parameters: RapiddocsGeneratorCli.ParameterReference[] = [];
    ...
    private getReferenceEndpointInvocationParameters({
        ...
        parameters: RapiddocsGeneratorCli.ParameterReference[];
    }): string {
        return `(${parameters
    ...
    private generateSdkContext(
        ...
        return new SdkContextImpl({
            ...
            dependencyManager: this.dependencyManager,
            rapiddocsConstants: this.intermediateRepresentation.constants,
            importsManager,
