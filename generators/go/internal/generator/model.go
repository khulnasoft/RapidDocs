...
func (f *fileWriter) WriteType(
	...
	visitor := &typeVisitor{
		...
		importPath:                   rapiddocsFilepathToImportPath(f.baseImportPath, typeDeclaration.Name.RapiddocsFilepath),
		writer:                       f,
...
func (t *typeReferenceVisitor) VisitNamed(named *ir.NamedType) error {
	...
	if importPath := rapiddocsFilepathToImportPath(t.baseImportPath, named.RapiddocsFilepath); importPath != t.importPath && t.packageLayout != PackageLayoutFlat {
...
func (c *singleUnionTypePropertiesVisitor) VisitSamePropertiesAsObject(named *ir.DeclaredTypeName) error {
	...
	name := named.Name.PascalCase.UnsafeName
	if importPath := rapiddocsFilepathToImportPath(c.baseImportPath, named.RapiddocsFilepath); importPath != c.importPath && c.packageLayout != PackageLayoutFlat {
...
// typeReferenceToUndiscriminatedUnionField maps Rapiddocs's type references to the field name used in an
...
// containerToUndiscriminatedUnionField maps Rapiddocs's container types to the field name used in an
// undiscriminated union.
...
// rapiddocsFilepathToImportPath maps the given Rapiddocs filepath to its
// Go import path.
func rapiddocsFilepathToImportPath(baseImportPath string, rapiddocsFilepath *ir.RapiddocsFilepath) string {
	var packages []string
...
// literalToUndiscriminatedUnionField maps Rapiddocs's literal types to the field name used in an
// undiscriminated union.
...
// primitiveToGoType maps Rapiddocs's primitive types to their Go-equivalent.
...
// primitiveToUndiscriminatedUnionField maps Rapiddocs's primitive types to the field name used in an
// undiscriminated union.
