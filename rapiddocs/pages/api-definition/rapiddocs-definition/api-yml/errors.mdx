...
In order to generate SDKs idiomatically, Rapiddocs needs to know how to differentiate
between different errors when parsing an endpoint response.

### Discriminate by status code

You can specify Rapiddocs to discriminate by status code. This means on each
...
You can specify Rapiddocs to discriminate by error name. If you select this strategy,
then Rapiddocs will assume that every error response has an extra property denoting
the error name.

If you use Rapiddocs to generate server-side code, then this option provides
