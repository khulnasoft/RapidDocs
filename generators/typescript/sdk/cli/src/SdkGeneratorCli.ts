import { AbstractGeneratorCli } from "@rapiddocs-typescript/abstract-generator-cli";
import {
    JavaScriptRuntime,
    NpmPackage,
...
import { SdkGenerator } from "@rapiddocs-typescript/sdk-generator";

import { RapiddocsGeneratorExec } from "@khulnasoft/base-generator";
import { AbsoluteFilePath } from "@khulnasoft/fs-utils";
import { getNamespaceExport } from "@khulnasoft/typescript-base";
...
export class SdkGeneratorCli extends AbstractGeneratorCli<SdkCustomConfig> {
    ...
    protected executionEnvironment(config: RapiddocsGeneratorExec.GeneratorConfig): "local" | "dev" | "prod" {
        return config.environment.type === "local"
            ? "local"
            : config.environment.coordinatorUrlV2.endsWith("dev2.buildwithrapiddocs.com")
              ? "dev"
              : "prod";
    }
}
