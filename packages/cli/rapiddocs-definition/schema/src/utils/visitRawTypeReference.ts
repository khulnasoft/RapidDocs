import { Literal, PrimitiveType, PrimitiveTypeV1, PrimitiveTypeV2 } from "@khulnasoft/ir-sdk";
...
export const RapiddocsContainerRegex = {
    MAP: /^map<\s*([^,]*)\s*,\s*(.*)\s*>$/,
    LIST: /^list<\s*(.*)\s*>$/,
    SET: /^set<\s*(.*)\s*>$/,
...
export function visitRawTypeReference<R>({
    ...
    const mapMatch = type.match(RapiddocsContainerRegex.MAP);
    ...
    const listMatch = type.match(RapiddocsContainerRegex.LIST);
    if (listMatch?.[1] != null) {
        return visitor.list(listMatch[1]);
    }

    const setMatch = type.match(RapiddocsContainerRegex.SET);
    if (setMatch?.[1] != null) {
        return visitor.set(setMatch[1]);
    }

    const optionalMatch = type.match(RapiddocsContainerRegex.OPTIONAL);
    if (optionalMatch?.[1] != null) {
        return visitor.optional(optionalMatch[1]);
    }

    const nullableMatch = type.match(RapiddocsContainerRegex.NULLABLE);
    if (nullableMatch?.[1] != null) {
        return visitor.nullable(nullableMatch[1]);
    }

    const literalMatch = type.match(RapiddocsContainerRegex.LITERAL);
