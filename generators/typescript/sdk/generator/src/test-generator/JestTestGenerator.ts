...
import { GeneratedSdkClientClass, SdkContext } from "@rapiddocs-typescript/contexts";
...
import * as IR from "@rapiddocs-rapiddocs/ir-sdk/api";
...
export class JestTestGenerator {
    ...
    private addJestConfig(): void {
        ...
        //             command: \`rapiddocs mock --port=\${PORT}\`,
    ...
    public getTestFile(serviceId: string, service: IR.HttpService): ExportedFilePath {
        const folders = service.name.rapiddocsFilepath.packagePath.map((folder) => folder.originalName);
        const filename = `${service.name.rapiddocsFilepath.file?.camelCase.unsafeName ?? "main"}.test.ts`;
    ...
    public get scripts(): Record<string, string> {
        if (this.writeUnitTests) {
            return {
                ...
                "wire:test": "npm install -g khulnasoft && rapiddocs test --command 'jest tests/wire'"
            };
        } else {
            return {
                test: "jest"
...
* to your SDK.
* Be sure to mark this file in \`.rapiddocsignore\`.
*
* If you include example requests/responses in your rapiddocs definition,
