#!/usr/bin/env node
import { RUNTIME } from "@rapiddocs-typescript/fetcher";
...
import { AbsoluteFilePath, cwd, doesPathExist, isURL, resolve } from "@khulnasoft/fs-utils";
import { initializeAPI, initializeDocs, initializeWithMintlify, initializeWithReadme } from "@khulnasoft/init";
import { LOG_LEVELS, LogLevel } from "@khulnasoft/logger";
import { askToLogin, login } from "@khulnasoft/login";
import { RapiddocsCliError, LoggableRapiddocsCliError } from "@khulnasoft/task-context";
...
const USE_NODE_18_OR_ABOVE_MESSAGE = "The Rapiddocs CLI requires Node 18+ or above.";
...
async function runCli() {
    ...
    try {
        const cwd = process.env[RAPIDDOCS_CWD_ENV_VAR];
        if (cwd != null) {
            process.chdir(cwd);
        }
        ...
        } else if (error instanceof RapiddocsCliError) {
            // thrower is responsible for logging, so we generally don't need to log here.
            cliContext.failWithoutThrowing();
        } else if (error instanceof LoggableRapiddocsCliError) {
...
async function getIntendedVersionOfCli(cliContext: CliContext): Promise<string> {
    if (process.env.RAPIDDOCS_NO_VERSION_REDIRECTION === "true") {
            ...
            if (argv.api != null && argv.docs != null) {
                ...
                await cliContext.runTask(async (context) => {
                    await initializeWithReadme({
                        ...
                        organization: argv.organization ?? "rapiddocs",
                        taskContext: context,
...
function addTokenCommand(cli: Argv<GlobalCliOptions>, cliContext: CliContext) {
    cli.command(
        "token",
        "Generate a Rapiddocs Token",
        (yargs) =>
            yargs.option("organization", {
                alias: "org",
                type: "string",
                description: "The organization to create a token for. Defaults to the one in `rapiddocs.config.json`"
            }),
        async (argv) => {
            await cliContext.runTask(async (context) => {
                await generateToken({
    ...
    cli.command(
        ...
        (yargs) =>
            yargs
                ...
                .option("instance", {
                    string: true,
                    description: "The url for the instance of docs (e.g. --instance acme.docs.buildwithrapiddocs.com)"
                })
                .option("preview", {
                    boolean: true,
                    default: false,
...
function addValidateCommand(cli: Argv<GlobalCliOptions>, cliContext: CliContext) {
    cli.command(
        "check",
        "Validates your Rapiddocs Definition. Logs errors.",
        (yargs) =>
            yargs
                .option("api", {
                    string: true,
...
function addLoginCommand(cli: Argv<GlobalCliOptions>, cliContext: CliContext) {
    cli.command(
        "login",
        "Log in to Rapiddocs via GitHub",
        (yargs) =>
...
function addFormatCommand(cli: Argv<GlobalCliOptions>, cliContext: CliContext) {
    cli.command(
        "format",
        "Formats your Rapiddocs Definition",
        (yargs) =>
            yargs
                .option("ci", {
                    boolean: true,
...
function addWriteOverridesCommand(cli: Argv<GlobalCliOptions>, cliContext: CliContext) {
    cli.command(
        ...
        async (argv) => {
            await cliContext.instrumentPostHogEvent({
                command: "rapiddocs generate-overrides"
            });
...
function addWriteDefinitionCommand(cli: Argv<GlobalCliOptions>, cliContext: CliContext) {
    cli.command(
        "write-definition",
        "Write underlying Rapiddocs Definition for OpenAPI specs and API Dependencies.",
        (yargs) =>
            yargs
                .option("api", {
                    string: true,
                ...
                .option("preserve-schemas", {
                    string: true,
                    description: "Preserve potentially unsafe schema Ids in the generated rapiddocs definition"
                }),
        async (argv) => {
            const preserveSchemaIds = argv.preserveSchemas != null;
            await cliContext.instrumentPostHogEvent({
                command: "rapiddocs write-definition"
            });
...
function addWriteDocsDefinitionCommand(cli: Argv<GlobalCliOptions>, cliContext: CliContext) {
    cli.command(
        ...
        async (argv) => {
            await cliContext.instrumentPostHogEvent({
                command: "rapiddocs write-docs-definition",
                properties: {
