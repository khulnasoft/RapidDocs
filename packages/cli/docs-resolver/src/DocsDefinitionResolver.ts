...
import { WithoutQuestionMarks, docsYml, parseDocsConfiguration } from "@khulnasoft/configuration-loader";
import { assertNever, isNonNullish, visitDiscriminatedUnion } from "@khulnasoft/core-utils";
import {
    parseImagePaths,
    replaceImagePathsAndUrls,
    replaceReferencedCode,
    replaceReferencedMarkdown
} from "@khulnasoft/docs-markdown-utils";
import { APIV1Write, DocsV1Write, FdrAPI, RapiddocsNavigation } from "@khulnasoft/fdr-sdk";
import {
    AbsoluteFilePath,
    RelativeFilePath,
    doesPathExist,
...
export class DocsDefinitionResolver {
    constructor(
        ...
        private rapiddocsWorkspaces: RapiddocsWorkspace[],
    ...
    public async resolve(): Promise<DocsV1Write.DocsDefinition> {
        this._parsedDocsConfig = await parseDocsConfiguration({
            ...
            absolutePathToRapiddocsFolder: this.docsWorkspace.absoluteFilePath,
            ...
            await visitNavigationAst({
                ...
                visitor: {
                    apiSection: async ({ workspace }) => {
                        ...
                        rapiddocsWorkspace.changelog?.files.forEach((file) => {
                ...
                rapiddocsWorkspaces: this.rapiddocsWorkspaces,
                context: this.taskContext
            });
        }
    ...
    private async getMarkdownFilesToFullyQualifiedPathNames(
        ...
        const markdownFilesToPathName: Record<AbsoluteFilePath, string> = {};
        const root = RapiddocsNavigation.migrate.RapiddocsNavigationV1ToLatest.create().root(await this.toRootNode());

        // all the page slugs in the docs:
        const collector = RapiddocsNavigation.NodeCollector.collect(root);
        collector.slugMap.forEach((node, slug) => {
            if (node == null || !RapiddocsNavigation.isPage(node)) {
                return;
            }

            const pageId = RapiddocsNavigation.getPageId(node);
            if (pageId == null) {
                return;
            }
    ...
    private getRapiddocsWorkspaceForApiSection(apiSection: docsYml.DocsNavigationItem.ApiSection): RapiddocsWorkspace {
        if (this.rapiddocsWorkspaces.length === 1 && this.rapiddocsWorkspaces[0] != null) {
            return this.rapiddocsWorkspaces[0];
        } else if (apiSection.apiName != null) {
            const rapiddocsWorkspace = this.rapiddocsWorkspaces.find((workspace) => {
                return workspace.workspaceName === apiSection.apiName;
            });
            if (rapiddocsWorkspace != null) {
                return rapiddocsWorkspace;
            }
        }
    ...
    private async toRootNode(): Promise<RapiddocsNavigation.V1.RootNode> {
        const slug = RapiddocsNavigation.V1.SlugGenerator.init(RapiddocsNavigation.slugjoin(this.getDocsBasePath()));
        const id = this.#idgen.get("root");

        const child: RapiddocsNavigation.V1.RootChild = await this.toRootChild(slug);
        ...
        return {
            ...
            viewers: undefined,
            orphaned: undefined,
            roles: this.parsedDocsConfig.roles?.map((role) => RapiddocsNavigation.RoleId(role)),
            featureFlags: undefined
        };
    }

    private async toRootChild(slug: RapiddocsNavigation.V1.SlugGenerator): Promise<RapiddocsNavigation.V1.RootChild> {
        return visitDiscriminatedUnion(this.parsedDocsConfig.navigation)._visit<Promise<RapiddocsNavigation.V1.RootChild>>({
            untabbed: (untabbed) =>
    ...
    ): RapiddocsNavigation.V1.LandingPageNode {
        const pageId = RapiddocsNavigation.PageId(this.toRelativeFilepath(landingPageConfig.absolutePath));
    ...
    private async toUnversionedNode({
        ...
        parentSlug: RapiddocsNavigation.V1.SlugGenerator;
    }): Promise<RapiddocsNavigation.V1.UnversionedNode> {
        const id = this.#idgen.get("unversioned");
        const landingPage: RapiddocsNavigation.V1.LandingPageNode | undefined =
    ...
    private async toVersionedNode(
        ...
        parentSlug: RapiddocsNavigation.V1.SlugGenerator
    ): Promise<RapiddocsNavigation.V1.VersionedNode> {
    ...
    private async toVersionNode(
        ...
        parentSlug: RapiddocsNavigation.V1.SlugGenerator,
        isDefault: boolean
    ): Promise<RapiddocsNavigation.V1.VersionNode> {
        ...
        return {
            type: "version",
            id,
            versionId: RapiddocsNavigation.VersionId(version.version),
    ...
    private async toSidebarRootNode(
        ...
        parentSlug: RapiddocsNavigation.V1.SlugGenerator
    ): Promise<RapiddocsNavigation.V1.SidebarRootNode> {
        ...
        const grouped: RapiddocsNavigation.V1.SidebarRootChild[] = [];
        children.forEach((child) => {
            ...
            let sidebarGroup: RapiddocsNavigation.V1.SidebarGroupNode;
    ...
    ): Promise<RapiddocsNavigation.V1.NavigationChild> {
        return visitDiscriminatedUnion(item)._visit<Promise<RapiddocsNavigation.V1.NavigationChild>>({
    ...
    private async toApiSectionNode(
        ...
        parentSlug: RapiddocsNavigation.V1.SlugGenerator,
        hideChildren?: boolean
    ): Promise<RapiddocsNavigation.V1.ApiReferenceNode> {
        ...
        let workspace: RapiddocsWorkspace | undefined = undefined;
        if (this.parsedDocsConfig.experimental?.openapiParserV3) {
            ...
            workspace = this.getRapiddocsWorkspaceForApiSection(item);
    ...
    private async toChangelogNode(
        ...
        parentSlug: RapiddocsNavigation.V1.SlugGenerator,
        hideChildren?: boolean
    ): Promise<RapiddocsNavigation.V1.ChangelogNode> {
    ...
    private async toLinkNode(item: docsYml.DocsNavigationItem.Link): Promise<RapiddocsNavigation.V1.LinkNode> {
        return {
            type: "link",
            id: this.#idgen.get(item.url),
            title: item.text,
            url: RapiddocsNavigation.V1.Url(item.url),
            icon: item.icon
        };
    }
    ...
    private async toPageNode(
        ...
        parentSlug: RapiddocsNavigation.V1.SlugGenerator,
        hideChildren?: boolean
    ): Promise<RapiddocsNavigation.V1.PageNode> {
        const pageId = RapiddocsNavigation.PageId(this.toRelativeFilepath(item.absolutePath));
        const slug = parentSlug.apply({
    ...
    private async toSectionNode(
        ...
        parentSlug: RapiddocsNavigation.V1.SlugGenerator,
        hideChildren?: boolean
    ): Promise<RapiddocsNavigation.V1.SectionNode> {
        const relativeFilePath = this.toRelativeFilepath(item.overviewAbsolutePath);
        const pageId = relativeFilePath ? RapiddocsNavigation.PageId(relativeFilePath) : undefined;
    ...
    private async convertTabbedNavigation(
        ...
        parentSlug: RapiddocsNavigation.V1.SlugGenerator
    ): Promise<RapiddocsNavigation.V1.TabbedNode> {
    ...
    private async toTabChild(
        ...
        parentSlug: RapiddocsNavigation.V1.SlugGenerator
    ): Promise<RapiddocsNavigation.V1.TabChild> {
        return visitDiscriminatedUnion(item.child)._visit<Promise<RapiddocsNavigation.V1.TabChild>>({
    ...
    private async toTabLinkNode(item: docsYml.TabbedNavigation, href: string): Promise<RapiddocsNavigation.V1.LinkNode> {
        return {
            type: "link",
            id: this.#idgen.get(href),
            title: item.title,
            url: RapiddocsNavigation.V1.Url(href),
            icon: item.icon
        };
    }
    ...
    private async toTabNode(
        ...
        parentSlug: RapiddocsNavigation.V1.SlugGenerator
    ): Promise<RapiddocsNavigation.V1.TabNode> {
...
function createEditThisPageUrl(
    editThisPage: docsYml.RawSchemas.RapiddocsDocsConfig.EditThisPageConfig | undefined,
    pageFilepath: string
): string | undefined {
    ...
    return `${wrapWithHttps(host)}/${owner}/${repo}/blob/${branch}/rapiddocs/${pageFilepath}?plain=1`;
}

function convertAvailability(
    availability: docsYml.RawSchemas.VersionAvailability
): RapiddocsNavigation.V1.NavigationV1Availability {
    switch (availability) {
        case "beta":
            return RapiddocsNavigation.V1.NavigationV1Availability.Beta;
        case "deprecated":
            return RapiddocsNavigation.V1.NavigationV1Availability.Deprecated;
        case "ga":
            return RapiddocsNavigation.V1.NavigationV1Availability.GenerallyAvailable;
        case "stable":
            return RapiddocsNavigation.V1.NavigationV1Availability.Stable;
        default:
