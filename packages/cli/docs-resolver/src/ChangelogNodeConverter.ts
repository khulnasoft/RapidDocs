...
import { APIV1Write, RapiddocsNavigation } from "@khulnasoft/fdr-sdk";
...
export class ChangelogNodeConverter {
    ...
    public toChangelogNode(opts: ConvertOptions): RapiddocsNavigation.V1.ChangelogNode {
        const title = opts.title ?? DEFAULT_CHANGELOG_TITLE;

        const unsortedChangelogItems: {
            date: Date;
            pageId: RapiddocsNavigation.PageId;
        ...
        for (const absoluteFilepath of this.changelogFiles ?? []) {
            ...
            unsortedChangelogItems.push({
                date: changelogDate,
                pageId: RapiddocsNavigation.PageId(relativePath),
                absoluteFilepath
            });
        }
        ...
        const changelogItems = unsortedChangelogItems.map((item): RapiddocsNavigation.V1.ChangelogEntryNode => {
        ...
        const overviewPageId =
            overviewPagePath != null ? RapiddocsNavigation.PageId(this.toRelativeFilepath(overviewPagePath)) : undefined;
    ...
    private groupByYear(
        prefix: string,
        entries: RapiddocsNavigation.V1.ChangelogEntryNode[],
        parentSlug: RapiddocsNavigation.V1.SlugGenerator
    ): RapiddocsNavigation.V1.ChangelogYearNode[] {
        const years = new Map<number, RapiddocsNavigation.V1.ChangelogEntryNode[]>();
    ...
    private groupByMonth(
        ...
        entries: RapiddocsNavigation.V1.ChangelogEntryNode[],
        parentSlug: RapiddocsNavigation.V1.SlugGenerator
    ): RapiddocsNavigation.V1.ChangelogMonthNode[] {
        const months = new Map<number, RapiddocsNavigation.V1.ChangelogEntryNode[]>();
        for (const entry of entries) {
