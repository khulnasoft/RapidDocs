...
Rapiddocs handles transforming an API definition -- either an OpenAPI or Rapiddocs specification -- into Rapiddocs _intermediate representation_. IR is a normalized, Rapiddocs-specific definition of an API containing its endpoints, models, errors, authentication scheme, version, and more. Then the TypeScript generator takes over and turns the IR into production-ready code.

## Generating TypeScript

This generator is used via the [Rapiddocs CLI](https://github.com/khulnasoft/rapiddocs), by defining one of the aforementioned TypeScript artifacts as a generator:
...
By default, the exported namespace and client are named based on the organization and API names in the Rapiddocs Definition.
...
when you utilize [`.rapiddocsignore`](https://buildwithrapiddocs.com/docs/compiler/rapiddocs-generate#rapiddocsignore) to
...
In Rapiddocs, there's an `unknown` type that represents data that isn't knowable at runtime. By default,
these types are generated into TypeScript as the `unknown` type.

When `treatUnknownAsAny` is enabled, `unknown` types from Rapiddocs are generated into TypeScript using `any`.
...
By default, Rapiddocs's `optional<>` properties will translate to optional TypeScript properties:
...
service defined in your Rapiddocs Definition.
...
By default, if you throw a non-Rapiddocs error in your endpoint handler, it will be caught by generated code
and a 500 response will be returned. No details from the error will be leaked to the client.

If `doNotHandleUnrecognizedErrors` is enabled and you throw a non-Rapiddocs error, the error will be caught
