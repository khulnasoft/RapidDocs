...
import { AbstractCsharpGeneratorContext, AsIsFiles, csharp } from "@khulnasoft/csharp-codegen";
...
} from "@rapiddocs-rapiddocs/ir-sdk/api";
...
export class SdkGeneratorContext extends AbstractCsharpGeneratorContext<SdkCustomConfigSchema> {
    ...
    public getDirectoryForTypeId(typeId: TypeId): RelativeFilePath {
        ...
        return RelativeFilePath.of(
            [
                ...typeDeclaration.name.rapiddocsFilepath.allParts.map((path) => path.pascalCase.safeName),
                TYPES_FOLDER_NAME
            ].join("/")
        );
    }
    ...
    public getDirectoryForError(declaredErrorName: DeclaredErrorName): RelativeFilePath {
        return RelativeFilePath.of(
            [
                ...declaredErrorName.rapiddocsFilepath.allParts.map((path) => path.pascalCase.safeName),
    ...
    public getNamespaceForTypeId(typeId: TypeId): string {
        const typeDeclaration = this.getTypeDeclarationOrThrow(typeId);
        return this.getNamespaceFromRapiddocsFilepath(typeDeclaration.name.rapiddocsFilepath);
    }

    public getAccessFromRootClient(rapiddocsFilepath: RapiddocsFilepath): string {
        const clientVariableName = this.getClientVariableName();
        const clientAccessParts = rapiddocsFilepath.allParts.map((part) => part.pascalCase.safeName);
    ...
    public getNamespaceForServiceId(serviceId: ServiceId): string {
        ...
        return this.getNamespaceFromRapiddocsFilepath(service.name.rapiddocsFilepath);
    }

    public getDirectoryForSubpackage(subpackage: Subpackage): string {
        return this.getDirectoryForRapiddocsFilepath(subpackage.rapiddocsFilepath);
    }

    public getDirectoryForServiceId(serviceId: ServiceId): string {
        const service = this.getHttpServiceOrThrow(serviceId);
        return this.getDirectoryForRapiddocsFilepath(service.name.rapiddocsFilepath);
    }

    public getDirectoryForRapiddocsFilepath(rapiddocsFilepath: RapiddocsFilepath): string {
        return RelativeFilePath.of([...rapiddocsFilepath.allParts.map((path) => path.pascalCase.safeName)].join("/"));
    }
    ...
    public getSubpackageClassReference(subpackage: Subpackage): csharp.ClassReference {
        return csharp.classReference({
            ...
            namespace: this.getNamespaceFromRapiddocsFilepath(subpackage.rapiddocsFilepath)
        });
    }
    ...
    public getExceptionClassReference(declaredErrorName: DeclaredErrorName): csharp.ClassReference {
        return csharp.classReference({
            ...
            namespace: this.getNamespaceFromRapiddocsFilepath(declaredErrorName.rapiddocsFilepath)
        });
    }
    ...
    public getRequestWrapperReference(serviceId: ServiceId, requestName: Name): csharp.ClassReference {
        const service = this.getHttpServiceOrThrow(serviceId);
        RelativeFilePath.of([...service.name.rapiddocsFilepath.allParts.map((path) => path.pascalCase.safeName)].join("/"));
        return csharp.classReference({
    ...
    override getChildNamespaceSegments(rapiddocsFilepath: RapiddocsFilepath): string[] {
        const segmentNames =
            this.customConfig["explicit-namespaces"] === true ? rapiddocsFilepath.allParts : rapiddocsFilepath.packagePath;
