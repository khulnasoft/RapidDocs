...
// unlike `RapiddocsNavigation.EndpointId`, which concatenates the subpackageId and endpointId with a dot,
...
export class ApiDefinitionHolder {
    ...
    #endpoints = new Map<RapiddocsNavigation.EndpointId, APIV1Read.EndpointDefinition>();
    #webSockets = new Map<RapiddocsNavigation.WebSocketId, APIV1Read.WebSocketChannel>();
    #webhooks = new Map<RapiddocsNavigation.WebhookId, APIV1Read.WebhookDefinition>();
    #endpointsInverted = new Map<APIV1Read.EndpointDefinition, RapiddocsNavigation.EndpointId>();
    #webSocketsInverted = new Map<APIV1Read.WebSocketChannel, RapiddocsNavigation.WebSocketId>();
    #webhooksInverted = new Map<APIV1Read.WebhookDefinition, RapiddocsNavigation.WebhookId>();
    ...
    get endpoints(): ReadonlyMap<RapiddocsNavigation.EndpointId, APIV1Read.EndpointDefinition> {
        return this.#endpoints;
    }

    get webSockets(): ReadonlyMap<RapiddocsNavigation.WebSocketId, APIV1Read.WebSocketChannel> {
        return this.#webSockets;
    }

    get webhooks(): ReadonlyMap<RapiddocsNavigation.WebhookId, APIV1Read.WebhookDefinition> {
    ...
    public getEndpointId(endpoint: APIV1Read.EndpointDefinition): RapiddocsNavigation.EndpointId | undefined {
        return this.#endpointsInverted.get(endpoint);
    }

    public getWebSocketId(webSocket: APIV1Read.WebSocketChannel): RapiddocsNavigation.WebSocketId | undefined {
        return this.#webSocketsInverted.get(webSocket);
    }

    public getWebhookId(webhook: APIV1Read.WebhookDefinition): RapiddocsNavigation.WebhookId | undefined {
    ...
    public static createEndpointId(
        ...
        subpackageId: string
    ): RapiddocsNavigation.EndpointId {
        return RapiddocsNavigation.EndpointId(endpoint.originalEndpointId ?? `${subpackageId}.${endpoint.id}`);
    }

    public static createWebSocketId(
        webSocket: APIV1Read.WebSocketChannel,
        subpackageId: string
    ): RapiddocsNavigation.WebSocketId {
        return RapiddocsNavigation.WebSocketId(`${subpackageId}.${webSocket.id}`);
    }

    public static createWebhookId(
        webhook: APIV1Read.WebhookDefinition,
        subpackageId: string
    ): RapiddocsNavigation.WebhookId {
        return RapiddocsNavigation.WebhookId(`${subpackageId}.${webhook.id}`);
    }
}
