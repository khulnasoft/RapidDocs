...
import { MediaType } from "@khulnasoft/core-utils";
import { DocsDefinitionResolver, UploadedFile, wrapWithHttps } from "@khulnasoft/docs-resolver";
import { AbsoluteFilePath, RelativeFilePath, convertToRapiddocsHostRelativeFilePath, resolve } from "@khulnasoft/fs-utils";
    ...
    const resolver = new DocsDefinitionResolver(
        ...
        async (files) => {
            ...
            [...measuredImages.values()].forEach((image) => {
                ...
                const imageFilePath = {
                    filePath: CjsFdrSdk.docs.v1.write.FilePath(
                        convertToRapiddocsHostRelativeFilePath(filePath.relativeFilePath)
                    ),
                    width: image.width,
                    height: image.height,
            ...
            const filepaths = files
                ...
                .map(({ relativeFilePath }) => convertToRapiddocsHostRelativeFilePath(relativeFilePath));
            ...
            if (response.ok) {
                ...
                switch (response.error.error) {
                    ...
                    default:
                        if (apiName != null) {
                            return context.failAndThrow(
                                `Failed to publish docs because API definition (${apiName}) could not be uploaded. Please contact support@buildwithrapiddocs.com\n ${response.error}`
                            );
                        } else {
                            return context.failAndThrow(
                                `Failed to publish docs because API definition could not be uploaded. Please contact support@buildwithrapiddocs.com\n ${response.error}`
                            );
                        }
                }
            }
...
async function startDocsRegisterFailed(
    ...
    switch (error.error) {
        case "InvalidCustomDomainError":
            return context.failAndThrow(
                `Your docs domain should end with ${process.env.DOCS_DOMAIN_SUFFIX ?? "docs.buildwithrapiddocs.com"}`
            );
        ...
        case "UnauthorizedError":
            return context.failAndThrow("Please make sure that your RAPIDDOCS_TOKEN is set.");
        case "UserNotInOrgError":
            return context.failAndThrow(
        ...
        case "UnavailableError":
            return context.failAndThrow(
                "Failed to publish docs. Please try again later or reach out to Rapiddocs support at support@buildwithrapiddocs.com."
            );
        default:
