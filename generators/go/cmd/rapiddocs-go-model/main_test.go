...
import (
	...
	builtin "github.com/khulnasoft/rapiddocs-go/internal/testdata/model/builtin/fixtures"
	custom "github.com/khulnasoft/rapiddocs-go/internal/testdata/model/custom/fixtures"
	enum "github.com/khulnasoft/rapiddocs-go/internal/testdata/model/enum/fixtures"
	undiscriminated "github.com/khulnasoft/rapiddocs-go/internal/testdata/model/undiscriminated/fixtures"
	union "github.com/khulnasoft/rapiddocs-go/internal/testdata/model/union/fixtures"
...
const (
	commandName       = "rapiddocs-go-model"
...
func TestLiteral(t *testing.T) {
	t.Run("object", func(t *testing.T) {
		...
		require.NoError(t, json.Unmarshal([]byte(`{"eighteen": "rapiddocs"}`), &value))
		assert.Equal(t, "rapiddocs", value.Eighteen())
		...
		assert.Equal(t, "rapiddocs", object["eighteen"])
	})

	t.Run("union", func(t *testing.T) {
		missingType := new(union.UnionWithLiteral)
		require.Error(t, json.Unmarshal([]byte(`{"value": "rapiddocs", "base": "base", "extended": "extended"}`), &missingType))

		missingValue := new(union.UnionWithLiteral)
		require.Error(t, json.Unmarshal([]byte(`{"type": "rapiddocs", "base": "base", "extended": "extended"}`), &missingValue))

		missingBase := new(union.UnionWithLiteral)
		require.Error(t, json.Unmarshal([]byte(`{"type": "rapiddocs", "value": "rapiddocs", "extended": "extended"}`), &missingBase))

		missingExtended := new(union.UnionWithLiteral)
		require.Error(t, json.Unmarshal([]byte(`{"type": "rapiddocs", "value": "rapiddocs", "base": "base"}`), &missingExtended))

		value := new(union.UnionWithLiteral)
		require.NoError(t, json.Unmarshal([]byte(`{"type": "rapiddocs", "value": "rapiddocs", "base": "base", "extended": "extended"}`), &value))
		assert.Equal(t, "extended", value.Extended())
		assert.Equal(t, "base", value.Base())
		assert.Equal(t, "rapiddocs", value.Rapiddocs())
		...
		assert.Equal(t, []byte(`{"base":"base","extended":"extended","type":"rapiddocs","value":"rapiddocs"}`), actualBytes)
	})
}
...
func TestUndiscriminatedUnion(t *testing.T) {
	...
	assert.Empty(t, request.Body.RapiddocsStringLiteral)

	unionLiteral := new(undiscriminated.Union)
	require.NoError(t, json.Unmarshal([]byte(`"rapiddocs"`), unionLiteral))

	// Test that the string takes precedence over the literal because
	// they aren't specified in the correct order.
	assert.Empty(t, unionLiteral.RapiddocsStringLiteral)
	assert.Equal(t, "rapiddocs", unionLiteral.String)

	unionWithLiteral := new(undiscriminated.UnionWithLiteral)
	require.NoError(t, json.Unmarshal([]byte(`"rapiddocs"`), unionWithLiteral))

	// Test that the literal is used as long as it's actually observed
	// on the wire.
	assert.Equal(t, "rapiddocs", unionWithLiteral.RapiddocsStringLiteral)
