...
Rapiddocs integrates with several API definition formats: 
...
<AccordionGroup>
  ...
  <Accordion
    title="Rapiddocs Definition (REST, Webhook, & WebSocket APIs)"
  >
    The Rapiddocs Definition is our take on a simpler API definition format. It is designed with **best-practices**, 
    supports **both RESTful and event-driven APIs**, and is optimized for **SDK generation**. 

    <Note>
      The Rapiddocs Definition is inspired from internal API Definition formats built at companies like 
    ...
    Check out an example Rapiddocs Definition below: 
...
Once you have an API definition, Rapiddocs will use it as an input to generate artifacts 
