    ...
    create: ({ workspace, logger }) => {
        ...
        const getRuleViolations = ({
            ...
            if (declarationsForName == null) {
                logger.error(
                    `Could not find declarations for name: ${declaredName}. This is an error in the Rapiddocs validator.`
                );
                return [];
            }
...
function getDeclarations(workspace: RapiddocsWorkspace): Declarations {
