...
export declare namespace RemoteTaskHandler {
    export interface Init {
        ...
        taskId: RapiddocsFiddle.remoteGen.RemoteGenTaskId;
...
export class RemoteTaskHandler {
    ...
    public async processUpdate(
        remoteTask: RapiddocsFiddle.remoteGen.Task | undefined
...
async function downloadZipForTask({
    ...
    // pipe to zip
    const tmpDir = await tmp.dir({ prefix: "rapiddocs", unsafeCleanup: true });
...
function convertLogLevel(logLevel: RapiddocsFiddle.LogLevel): LogLevel {
    switch (logLevel) {
        case "DEBUG":
