...
import { EndpointWithExample, PrimitiveSchemaValueWithExample, SchemaWithExample, Source } from "@khulnasoft/openapi-ir";
...
import { getRapiddocsAvailability } from "../../extensions/getRapiddocsAvailability";
...
export function convertHttpOperation({
    ...
    const idempotent = getExtension<boolean>(operation, RapiddocsOpenAPIExtension.IDEMPOTENT);
    const requestNameOverride = getExtension<string>(operation, [
        RapiddocsOpenAPIExtension.REQUEST_NAME_V1,
        RapiddocsOpenAPIExtension.REQUEST_NAME_V2
    ]);
    ...
    // Rapiddocs doesn't support GET requests with bodies
    if (operationContext.method === "GET") {
        convertedRequest = undefined;
    }

    const availability = getRapiddocsAvailability(operation);
    const examples = getExamplesFromExtension(operationContext, operation, context);
    const serverName = getExtension<string>(operation, RapiddocsOpenAPIExtension.SERVER_NAME_V2);
    return {
        summary: operation.summary,
        internal: getExtension<boolean>(operation, OpenAPIExtension.INTERNAL),
        idempotent,
        audiences: getExtension<string[]>(operation, RapiddocsOpenAPIExtension.AUDIENCES) ?? [],
        operationId:
