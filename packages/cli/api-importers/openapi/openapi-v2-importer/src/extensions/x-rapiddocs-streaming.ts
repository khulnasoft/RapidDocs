...
export declare namespace RapiddocsStreamingExtension {
...
export class RapiddocsStreamingExtension extends AbstractExtension<
    OpenAPIConverterContext3_1,
    RapiddocsStreamingExtension.Output
> {
    private readonly operation: object;
    public readonly key = "x-rapiddocs-streaming";

    constructor({ breadcrumbs, operation }: RapiddocsStreamingExtension.Args) {
        super({ breadcrumbs });
    ...
    public convert({
    ...
    }): RapiddocsStreamingExtension.Output | undefined {
        ...
        if (!result.success) {
            errorCollector.collect({
                message: `Invalid x-rapiddocs-streaming extension: ${result.error.message}`,
