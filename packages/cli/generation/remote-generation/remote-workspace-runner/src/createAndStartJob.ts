...
import { createFiddleService, getFiddleOrigin } from "@khulnasoft/core";
import { AbsoluteFilePath, RelativeFilePath, dirname, join, stringifyLargeObject } from "@khulnasoft/fs-utils";
import {
...
import { Fetcher } from "@rapiddocs-rapiddocs/fiddle-sdk/core";
...
}): Promise<RapiddocsFiddle.remoteGen.CreateJobResponse> {
...
async function createJob({
    ...
    const generatorConfig: RapiddocsFiddle.GeneratorConfigV2 = {
    ...
    let rapiddocsDefinitionMetadata: RapiddocsFiddle.remoteGen.RapiddocsDefinitionMetadata | undefined;
    ...
    if (generatorInvocation.outputMode.type.startsWith("github")) {
        try {
            ...
            context.logger.debug(`Writing mock rapiddocs definition to ${absolutePathToTmpDir}`);

            const absolutePathToTmpRapiddocsDirectory = join(absolutePathToTmpDir, RelativeFilePath.of(RAPIDDOCS_DIRECTORY));
            const absolutePathToTmpDefinitionDirectory = join(
                absolutePathToTmpRapiddocsDirectory,
            ...
            // write rapiddocs.config.json
            const absolutePathToRapiddocsConfigJson = join(
                absolutePathToTmpRapiddocsDirectory,
                RelativeFilePath.of(PROJECT_CONFIG_FILENAME)
            );
            await writeFile(absolutePathToRapiddocsConfigJson, JSON.stringify(projectConfig.rawConfig, undefined, 2));
            // write sources
            // TODO: We need handle what happens with source files outside of the rapiddocs directory
            try {
            ...
            await createTar({ file: tarPath, cwd: absolutePathToTmpRapiddocsDirectory }, ["."]);

            // Upload definition to S3
            context.logger.debug("Getting upload URL for Rapiddocs definition.");
    ...
    if (!createResponse.ok) {
        return convertCreateJobError(createResponse.error as unknown as Fetcher.Error)._visit({
            ...
            orgNotConfiguredForWhitelabel: () => {
                return context.failAndThrow(
                    "Your org is not configured for white-labeling. Please reach out to support@buildwithrapiddocs.com."
                );
            },
            _other: (content) => {
...
async function startJob({
    ...
    intermediateRepresentation: IntermediateRepresentation;
    generatorInvocation: generatorsYml.GeneratorInvocation;
    job: RapiddocsFiddle.remoteGen.CreateJobResponse;
    context: TaskContext;
    irVersionOverride: string | undefined;
}): Promise<void> {
...
function convertCreateJobError(error: any): RapiddocsFiddle.remoteGen.createJobV3.Error {
    if (error?.reason === "status-code") {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const body = error.body as any;
        switch (body?._error) {
            case "IllegalApiNameError":
                return RapiddocsFiddle.remoteGen.createJobV3.Error.illegalApiNameError();
            case "GeneratorsDoNotExistError":
                return RapiddocsFiddle.remoteGen.createJobV3.Error.generatorsDoNotExistError(body.body);
            case "CannotPublishToNpmScope":
                return RapiddocsFiddle.remoteGen.createJobV3.Error.cannotPublishToNpmScope(body.body);
            case "CannotPublishToMavenScope":
                return RapiddocsFiddle.remoteGen.createJobV3.Error.cannotPublishToMavenGroup(body.body);
            case "InsufficientPermissions":
                return RapiddocsFiddle.remoteGen.createJobV3.Error.insufficientPermissions(body.body);
        }
    }
    return error;
}
...
function shouldUploadToS3({
    ...
    outputMode: RapiddocsFiddle.OutputMode;
    generatorInvocation: generatorsYml.GeneratorInvocation;
    absolutePathToPreview: AbsoluteFilePath | undefined;
    shouldLogS3Url: boolean;
}): boolean {
