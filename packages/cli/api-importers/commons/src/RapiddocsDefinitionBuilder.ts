...
import { RAPIDDOCS_PACKAGE_MARKER_FILENAME, ROOT_API_FILENAME } from "@khulnasoft/configuration";
import { RawSchemas, RootApiFileSchema, visitRawEnvironmentDeclaration } from "@khulnasoft/rapiddocs-definition-schema";
import { AbsoluteFilePath, RelativeFilePath, basename, dirname, join, relative } from "@khulnasoft/path-utils";
...
export interface RapiddocsDefinitionBuilder {
...
export class RapiddocsDefinitionBuilderImpl implements RapiddocsDefinitionBuilder {
    private root: RapiddocsDefinitionDirectory;
    ...
    public constructor(public readonly enableUniqueErrorsPerEndpoint: boolean) {
        this.root = new RapiddocsDefinitionDirectory();
        this.rootApiFile = {
            name: "api",
    ...
    public setServiceInfo(
        ...
        const rapiddocsFile = this.getOrCreateFile(file);
        if (rapiddocsFile.service == null) {
            // Set to default values if service is null
            rapiddocsFile.service = {
                auth: false,
                "base-path": "",
                endpoints: {}
            };
        ...
        if (auth != null) {
            rapiddocsFile.service.auth = auth;
        }
        if (basePath != null) {
            rapiddocsFile.service["base-path"] = basePath;
        }
        if (displayName != null) {
            rapiddocsFile.service["display-name"] = displayName;
        }
        if (docs != null) {
            rapiddocsFile.docs = docs;
        }
    }
    ...
    public addImport({
        ...
        const rapiddocsFile = this.getOrCreateFile(file);
        if (rapiddocsFile.imports == null) {
            rapiddocsFile.imports = {};
        }
        rapiddocsFile.imports[importPrefix] = relative(
            dirname(AbsoluteFilePath.of(`/${file}`)),
            AbsoluteFilePath.of(`/${fileToImport}`)
        );
        return importPrefix;
    ...
    public addType(
        ...
        const rapiddocsFile = this.getOrCreateFile(file);
        if (rapiddocsFile.types == null) {
            rapiddocsFile.types = {};
        }
        rapiddocsFile.types[name] = schema;
    }
    ...
    public addTypeExample(file: RelativeFilePath, name: string, convertedExample: RawSchemas.ExampleTypeSchema): void {
        ...
        const type = rapiddocsFile.types[name];
        if (type != null) {
            if (typeof type === "string") {
                rapiddocsFile.types[name] = {
                    type,
    ...
    public addError(
        ...
        const rapiddocsFile = this.getOrCreateFile(file);
        if (rapiddocsFile.errors == null) {
            rapiddocsFile.errors = {};
        }
        if (rapiddocsFile.errors[name] == null) {
            rapiddocsFile.errors[name] = schema;
        } else if (rapiddocsFile.errors[name]?.type !== schema.type) {
            rapiddocsFile.errors[name] = {
    ...
    public addErrorExample(
        ...
        const errorDeclaration = rapiddocsFile.errors[name];
    ...
    public addEndpoint(
        ...
        if (source != null) {
            rapiddocsFile.service.source = source;
        }
        rapiddocsFile.service.endpoints[name] = schema;
    }
    ...
    public addWebhook(
        ...
        const rapiddocsFile = this.getOrCreateFile(file);
        if (rapiddocsFile.webhooks == null) {
            rapiddocsFile.webhooks = {};
        }
        rapiddocsFile.webhooks[name] = schema;
    }

    public addChannel(file: RelativeFilePath, { channel }: { channel: RawSchemas.WebSocketChannelSchema }): void {
        const rapiddocsFile = this.getOrCreateFile(file);
        rapiddocsFile.channel = channel;

        const basePath = this.basePath;
        if (basePath != null) {
            rapiddocsFile.channel.path = join(basePath, channel.path);
        }
    }
    ...
    public addChannelExample(
        ...
        const rapiddocsFile = this.getOrCreateFile(file);
        if (rapiddocsFile.channel == null) {
            rapiddocsFile.channel = {
                path: "",
                auth: false
            };
        }
        if (rapiddocsFile.channel.messages == null) {
            rapiddocsFile.channel.messages = {};
        }
        if (rapiddocsFile.channel.examples == null) {
            rapiddocsFile.channel.examples = [];
        }
        rapiddocsFile.channel.examples.push(example);
    }
    ...
    public addChannelMessage(
        ...
        rapiddocsFile.channel.messages[messageId] = message;
    }

    public build(): RapiddocsDefinition {
        ...
        const definition: RapiddocsDefinition = {
    ...
    public getOrCreateFile(
        ...
        if (file === RAPIDDOCS_PACKAGE_MARKER_FILENAME) {
