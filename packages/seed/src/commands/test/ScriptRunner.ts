...
export class ScriptRunner {
    ...
    private async buildRapiddocsCli(context: TaskContext): Promise<AbsoluteFilePath> {
        const rootDir = join(AbsoluteFilePath.of(__dirname), RelativeFilePath.of("../../.."));
        await loggingExeca(context.logger, "pnpm", ["rapiddocs:build"], { cwd: rootDir });
    ...
    private async startContainers(context: TaskContext): Promise<void> {
        const absoluteFilePathToRapiddocsCli = await this.buildRapiddocsCli(context);
        const cliVolumeBind = `${absoluteFilePathToRapiddocsCli}:/rapiddocs`;
