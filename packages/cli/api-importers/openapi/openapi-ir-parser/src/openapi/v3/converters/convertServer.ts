...
import { Server } from "@khulnasoft/openapi-ir";
...
export function convertServer(server: OpenAPIV3.ServerObject): Server {
    return {
        ...
        audiences: getExtension<string[]>(server, RapiddocsOpenAPIExtension.AUDIENCES)
    };
}
...
function getServerName(server: OpenAPIV3.ServerObject): string | undefined {
    const name = getExtension<string>(server, [
        RapiddocsOpenAPIExtension.SERVER_NAME_V1,
        RapiddocsOpenAPIExtension.SERVER_NAME_V2
    ]);
