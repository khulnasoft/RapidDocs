...
from rapiddocs_python.codegen import AST, LocalClassReference, SourceFile
from rapiddocs_python.codegen.ast.references.class_reference import ClassReference
from rapiddocs_python.generators.pydantic_model.type_declaration_handler.abc.abstract_type_snippet_generator import (
    AbstractTypeSnippetGenerator,
)
from rapiddocs_python.pydantic_codegen import PydanticField
from rapiddocs_python.pydantic_codegen.pydantic_field import RapiddocsAwarePydanticField
...
class AbstractSimpleDiscriminatedUnionGenerator(AbstractTypeGenerator, ABC):
    ...
    def __init__(
        ...
        self._base_class_properties: List[RapiddocsAwarePydanticField] = []
    ...
    def generate(self) -> None:
        ...
        for single_union_type in self._union.types:
            ...
            if shape.properties_type == "singleProperty":
                ...
                single_property_property_rapiddocs_aware_fields: List[RapiddocsAwarePydanticField] = [
                    RapiddocsAwarePydanticField(
                        name=shape.name.name.snake_case.safe_name,
                        pascal_case_field_name=shape.name.name.pascal_case.safe_name,
                        json_field_name=shape.name.wire_value,
                        type_reference=shape.type,
                ...
                single_union_type_references.append(
                    self._generate_single_property_member(
                        ...
                        rapiddocs_aware_properties=single_property_property_rapiddocs_aware_fields,
                    )
                )

            elif shape.properties_type == "samePropertiesAsObject":
                same_properties_as_object_property_fields: List[RapiddocsAwarePydanticField] = [
                    RapiddocsAwarePydanticField(
                        name=get_discriminant_parameter_name(self._union.discriminant),
                        pascal_case_field_name=self._union.discriminant.name.pascal_case.safe_name,
                        type_reference=ir_types.TypeReference.factory.container(
                ...
                no_properties_property_field = RapiddocsAwarePydanticField(
                    name=get_discriminant_parameter_name(self._union.discriminant),
                    pascal_case_field_name=self._union.discriminant.name.pascal_case.safe_name,
                    type_reference=ir_types.TypeReference.factory.container(
                        ir_types.ContainerType.factory.literal(
    ...
    def _generate_single_property_member(
        ...
        rapiddocs_aware_properties: List[RapiddocsAwarePydanticField],
    ) -> LocalClassReference:
        ...

    @abstractmethod
    def _generate_same_properties_as_object_member(
        self, member_type_id: ir_types.TypeId, class_name: str, properties: List[RapiddocsAwarePydanticField]
    ) -> LocalClassReference:
        ...

    @abstractmethod
    def _generate_no_property_member(
        self, class_name: str, discriminant_field: RapiddocsAwarePydanticField
    ) -> LocalClassReference:
        ...
