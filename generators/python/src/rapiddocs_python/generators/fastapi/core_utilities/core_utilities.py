...
from rapiddocs_python.codegen import AST, ExportStrategy, Filepath, Project
from rapiddocs_python.external_dependencies.pydantic import (
...
from rapiddocs_python.generators.pydantic_model.field_metadata import FieldMetadata
from rapiddocs_python.source_file_factory import SourceFileFactory


class RapiddocsHTTPException:
    CLASS_NAME = "RapiddocsHTTPException"
    STATUS_CODE_MEMBER = "status_code"
    NAME_MEMBER = "name"
    CONTENT_MEMBER = "content"

    def __init__(self, filepath: Tuple[Filepath.DirectoryFilepathPart, ...]):
        self.filepath = Filepath(directories=filepath, file=Filepath.FilepathPart(module_name="rapiddocs_http_exception"))
    ...
    def get_reference_to(self) -> AST.ClassReference:
        return AST.ClassReference(
            ...
            import_=AST.ReferenceImport(
                ...
                named_import=RapiddocsHTTPException.CLASS_NAME,
            ),
        )
    ...
    def create(
        ...
        *,
        ...
        kwargs.append((RapiddocsHTTPException.STATUS_CODE_MEMBER, status_code))
        if name is not None:
            kwargs.append((RapiddocsHTTPException.NAME_MEMBER, name))
        if content is not None:
            kwargs.append((RapiddocsHTTPException.CONTENT_MEMBER, content))
...
class Exceptions:
    def __init__(self, filepath: Tuple[Filepath.DirectoryFilepathPart, ...]):
        ...
        self.RapiddocsHTTPException = RapiddocsHTTPException(filepath=self.filepath)

    def rapiddocs_http_exception_handler(self) -> AST.Reference:
        return AST.Reference(
            qualified_name_excluding_import=(),
            import_=AST.ReferenceImport(
                module=AST.Module.local(*self._module_path),
                named_import="rapiddocs_http_exception_handler",
            ),
        )
...
class CoreUtilities:
    ...
    def copy_to_project(self, *, project: Project) -> None:
        self._copy_file_to_project(
            project=project,
            relative_filepath_on_disk="abstract_rapiddocs_service.py",
            filepath_in_project=Filepath(
                directories=self.filepath,
                file=Filepath.FilepathPart(module_name="abstract_rapiddocs_service"),
            ),
            exports=set(),
        )
        self._copy_file_to_project(
    ...
    def _copy_exceptions_to_project(self, *, project: Project) -> None:
        self._copy_file_to_project(
            ...
            exports={"default_exception_handler", "rapiddocs_http_exception_handler", "http_exception_handler"},
        )
        self._copy_file_to_project(
            project=project,
    ...
    def AbstractRapiddocsService(self) -> AST.ClassReference:
        return AST.ClassReference(
            qualified_name_excluding_import=(),
            import_=AST.ReferenceImport(
                module=AST.Module.local(*self._module_path, "abstract_rapiddocs_service"),
                named_import="AbstractRapiddocsService",
            ),
        )

    INIT_RAPIDDOCS_METHOD_NAME = "_init_rapiddocs"
