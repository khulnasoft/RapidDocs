...
from rapiddocs_python.external_dependencies.starlette import Starlette
from rapiddocs_python.generators.fastapi.service_generator.endpoint_parameters.request.file_upload_request_endpoint_parameter import (
...
class EndpointGenerator:
    ...
    def get_endpoint_dot_delimited_path(self) -> str:
        service_path = [part.original_name for part in self._service.name.rapiddocs_filepath.all_parts]
    ...
    def _get_write_init_body(self, class_declaration: AST.ClassDeclaration) -> AST.CodeWriterFunction:
        def _write_init_body(writer: AST.NodeWriter) -> None:
            ...
            with writer.indent():
                ...
                writer.write("**")
                default_tag = ".".join(
                    [package.snake_case.safe_name for package in self._service.name.rapiddocs_filepath.all_parts]
                )
                writer.write_node(
    ...
    def _write_try_except_wrapper_body(self, writer: AST.NodeWriter) -> None:
        ...
        writer.write("except ")
        writer.write_reference(self._context.core_utilities.exceptions.RapiddocsHTTPException.get_reference_to())
        writer.write_line(f" as {CAUGHT_ERROR_NAME}:")
        with writer.indent():
            ...
            with writer.indent():
                ...
                writer.write_line('+ "the endpoint\'s errors list in your Rapiddocs Definition."')
            writer.write_line(")")
