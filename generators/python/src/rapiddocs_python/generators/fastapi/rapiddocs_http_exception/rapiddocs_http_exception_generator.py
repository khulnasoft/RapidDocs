from rapiddocs.ir.resources import NameAndWireValue

from rapiddocs_python.codegen import AST, LocalClassReference, Project, SourceFile
from rapiddocs_python.external_dependencies import FastAPI
from rapiddocs_python.generator_exec_wrapper import GeneratorExecWrapper
from rapiddocs_python.generators.fastapi.custom_config import FastAPICustomConfig
from rapiddocs_python.pydantic_codegen import PydanticField, PydanticModel

from ..context import FastApiGeneratorContext


class RapiddocsHTTPExceptionGenerator:
    _BODY_CLASS_NAME = "Body"

    def __init__(self, context: FastApiGeneratorContext, custom_config: FastAPICustomConfig):
        self._context = context
        self._custom_config = custom_config
        self.RapiddocsHTTPException = context.core_utilities.exceptions.RapiddocsHTTPException
    ...
    def generate(self, project: Project, generator_exec_wrapper: GeneratorExecWrapper) -> None:
        source_file = self._context.source_file_factory.create(
            ...
            filepath=self.RapiddocsHTTPException.filepath,
        )
        class_declaration = AST.ClassDeclaration(
            name=self.RapiddocsHTTPException.CLASS_NAME,
            is_abstract=True,
        ...
        project.write_source_file(source_file=source_file, filepath=self.RapiddocsHTTPException.filepath)
            ...
            signature=AST.FunctionSignature(
                parameters=[
                    AST.FunctionParameter(
                        name=self.RapiddocsHTTPException.STATUS_CODE_MEMBER, type_hint=AST.TypeHint.int_()
                    ),
                    AST.FunctionParameter(
                        name=self.RapiddocsHTTPException.NAME_MEMBER,
                        type_hint=AST.TypeHint.optional(AST.TypeHint.str_()),
                        initializer=AST.Expression("None"),
                    ),
                    AST.FunctionParameter(
                        name=self.RapiddocsHTTPException.CONTENT_MEMBER,
    ...
    def _write_constructor_body(self, writer: AST.NodeWriter) -> None:
        writer.write_line(f"super().__init__(status_code={self.RapiddocsHTTPException.STATUS_CODE_MEMBER})")
        writer.write_line(f"self.{self.RapiddocsHTTPException.NAME_MEMBER} = {self.RapiddocsHTTPException.NAME_MEMBER}")
        writer.write_line(
            f"self.{self.RapiddocsHTTPException.STATUS_CODE_MEMBER} = {self.RapiddocsHTTPException.STATUS_CODE_MEMBER}"
        )
        writer.write_line(f"self.{self.RapiddocsHTTPException.CONTENT_MEMBER} = {self.RapiddocsHTTPException.CONTENT_MEMBER}")
    ...
    def _construct_pydantic_model(
        ...
        with PydanticModel(
            ...
            name=RapiddocsHTTPExceptionGenerator._BODY_CLASS_NAME,
        ...
        def write_body(writer: AST.NodeWriter) -> None:
                ...
                AST.ClassInstantiation(
                    ...
                    kwargs=[
                        (
                            ...
                            AST.Expression("self." + self.RapiddocsHTTPException.NAME_MEMBER),
                        ),
                        (
            ...
            writer.write_node(
                FastAPI.JSONResponse.invoke(
                    ...
                    status_code=AST.Expression(AST.CodeWriter(f"self.{self.RapiddocsHTTPException.STATUS_CODE_MEMBER}")),
                )
            )
        ...
        def write_body(writer: AST.NodeWriter) -> None:
            ...
            writer.write_node(
                FastAPI.jsonable_encoder(
                    AST.Expression(f"self.{self.RapiddocsHTTPException.CONTENT_MEMBER}"), exclude_none=True
                )
            )
            writer.write_line()
    ...
    def _write_content_initializer(self, writer: AST.NodeWriter) -> None:
        writer.write(f"self.{self.RapiddocsHTTPException.CONTENT_MEMBER} or ")
        ...
        writer.write(f"(self.{self.RapiddocsHTTPException.STATUS_CODE_MEMBER}).phrase")
